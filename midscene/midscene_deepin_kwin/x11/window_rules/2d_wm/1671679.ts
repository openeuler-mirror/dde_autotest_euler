 ...
 * 用例 PMSID: 1671679
 * 用例标题：窗口级菜单—移至左边工作区—在第一个工作区移至左边工作区置灰
 * 生成时间: 2026-01-13 09:56:00
 * 用例编写人: UT006165（李日华）
 */

describe('1671679-窗口级菜单—移至左边工作区—在第一个工作区移至左边工作区置灰', () => {
  ...
  test('1671679-窗口级菜单—移至左边工作区—在第一个工作区移至左边工作区置灰', async ({ device, agent, uos }) => {
    // 前置条件：当前工作区数量大于等于2，当前在第1个工作区
    ...
    // 键入"alt+space"组合键，鼠标点击"移至左边工作区"
    await device.pressKey("Alt", "Space");
    await agent.aiAssert("窗口右键菜单已显示");
    
    // 预期结果1："移至左边的工作区"选项置灰无法点击
    await agent.aiAssert('窗口菜单中的"移至左边的工作区"选项无法点击');
  }, { timeout: 600000, tags: ['1671679', 'level2','x11','window_rules','2d_wm'] });
