 ...
 * 用例 PMSID: 1671691
 * 用例标题: 验证当工作区数量为4时，Ctrl+Alt+左箭头能够从右向左切换工作区
 * 生成时间: 2025-04-24 19:50:26
 * 用例编写人：UT006165(李日华)
 */

describe('1671691-验证当工作区数量为4时，Ctrl+Alt+左箭头能够从右向左切换工作区', () => {
  ...
  test('1671691-验证当工作区数量为4时，Ctrl+Alt+左箭头能够从右向左切换工作区', async ({ device, agent, uos }) => {
    // 将工作区减为1
    ...
    // 步骤 11: 执行Ctrl+Alt+左箭头切换测试
    console.log('步骤 11: 执行Ctrl+Alt+左箭头切换测试');
    console.log('========================================');
    ...
    // ==================== 验证切换成功 ====================
    console.log('\n========================================');
    console.log('验证Ctrl+Alt+左箭头切换功能');
    console.log('========================================');

    // 验证当前在工作区1（最左侧）- 使用简单断言避免base64错误
    console.log('\n--- 验证已切换到工作区1 ---');
    ...
    await agent.logScreenshot('验证工作区位置', {
      content: '验证当前在工作区1（最左侧）'
    });
    
    // 验证第一个工作区是激活状态（有高亮边框或选中效果）
    try {
      await agent.aiAssert(
        `屏幕顶部第一行（y坐标0到${workspaceAreaHeight}像素范围内）最左侧的工作区缩略图处于选中或激活状态。只分析这个顶部区域。`,
        undefined,
        { bbox: topBbox }
      );
      console.log('✓ 已成功切换到工作区1（最左侧工作区处于激活状态）');
    } catch (e) {
    ...
    console.log('\n========================================');
    console.log('测试完成：四工作区下Ctrl+Alt+左箭头切换测试通过');
    console.log('已验证：工作区4->3->2->1 依次切换成功');

  }, { timeout: 600000, tags: ['1671691', 'level1','x11','window_rules','2d_wm'] });
