 ...
 * 用例 PMSID: 1671683
 * 用例标题：窗口级菜单—最小化
 * 生成时间: 2026-01-07 11:19:00
 * 用例编写人: UT006165（李日华）
 */

describe('1671683-窗口级菜单—最小化', () => {
  ...
  test('1671683-窗口级菜单—最小化', async ({ device, agent, uos }) => {
    // 前置条件：第一个工作区内打开文件管理器应用
    await device.pressKey("Ctrl", "Alt", "1");
    await uos.openApp("文件管理器");
    await agent.aiWaitFor("文件管理器窗口已打开");
    
    // 步骤：键入"alt+space"组合键调出窗口右键菜单后，鼠标点击"最小化"选项
  ...
  }, { timeout: 600000, tags: ['1671683', 'level1','x11','window_rules','2d_wm'] });
  ...
  afterAll(async ({ uos, agent, device, system }) => {
    ...
    // 确保文件管理器窗口关闭
    await agent.aiRightClick("任务文件管理器图标");
