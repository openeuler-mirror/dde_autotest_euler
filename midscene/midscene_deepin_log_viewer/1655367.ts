...
describe('1655367-刷新频率——每1分钟', () => {
  ...
  test('1655367-刷新频率——每1分钟', async ({ device, agent, uos, system }) => {
    ...
    // 步骤 4: 点击每1分钟，每1分钟被选中，菜单窗口和刷新频率子菜单均关闭
    await agent.aiTap("每1分钟");
    await agent.aiWaitFor("菜单窗口和刷新频率子菜单均关闭");
    await agent.aiAssert("菜单窗口和刷新频率子菜单均关闭");

    // 步骤 5: 等待1分钟，查看日志收集工具窗口，会刷新界面
    // 循环检测刷新图标，最多等待 65 秒
    ...
    // 步骤 7: 再次打开菜单，检查“每1分钟”是否被选中
    await agent.aiTap("窗口菜单栏右侧三条横线的菜单项");
    await agent.aiWaitFor("弹出菜单窗口");
    await agent.aiTap("刷新频率");
    await agent.aiWaitFor("刷新频率子菜单");
    await agent.aiAssert("每1分钟选项处于被选中状态");

    // 步骤 8: 等待1分钟，查看日志收集工具窗口，会刷新界面
    // 循环检测刷新图标，最多等待 65 秒
    const startTime1 = Date.now();
    while (Date.now() - startTime1 < 65000) {
      const isRefreshing = await agent.aiBoolean("日志收集工具窗口主界面中部内容刷新显示出来");
      if (isRefreshing) {
        isRefreshed = true;
        break;
  ...
  }, { timeout: 700000, tags: ['1655367', 'level1', 'smoke'] });
