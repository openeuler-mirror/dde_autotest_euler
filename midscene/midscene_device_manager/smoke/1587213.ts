...
describe('1587213-内存不可禁用', () => {
    ...
    test('1587213-内存不可禁用', async ({ device, agent, uos, system }) => {
        ...
        await agent.aiTap('左侧菜单列中的内存分类');
        await agent.aiWaitFor('切换到了内存界面');
        await agent.aiRightClick('制造商下方的记录信息');
        await agent.aiAssert('无禁用菜单命令');
        await device.pressKey('ESC');
        await uos.closeCurrentWindow();
    }, { timeout: 600000, tags: ["1587213", "level1", "smoke", "xuqi"] });
