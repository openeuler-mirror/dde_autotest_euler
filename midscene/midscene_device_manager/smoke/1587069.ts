...
describe('1587069-调节系统字号为最大-应用界面显示', () => {
    beforeAll(async ({ device, uos, system, agent }) => {
        console.log('1. beforeAll: 初始化测试套件');
        await uos.showDesktop();
        await system.exec(`dde-dconfig set org.deepin.dde.appearance -r org.deepin.dde.appearance Font_Size -v 15`);
    ...
    test('1587069-调节系统字号为最大-应用界面显示', async ({ device, agent, uos, system }) => {
        ...
        await agent.aiAssert('设备管理器界面展示了内容');
        await uos.closeCurrentWindow();
    }, { timeout: 600000, tags: ["1587069", "level1", "smoke", "xuqi"] });
