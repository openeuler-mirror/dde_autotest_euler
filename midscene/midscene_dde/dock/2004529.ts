/**
 * 用例 PMSID: 2004529
 * 用例标题:【任务栏】【固定区域】【UOS AI】小U同学插件图标多次左键点击响应正常
 * 生成时间: 2026-05-29 11:00:00
 * 用例编写人:UT000327(秦家喜)
 */

describe('2004529-【任务栏】【固定区域】【UOS AI】小U同学插件图标多次左键点击响应正常', () => {
    ...
    test('2004529-【任务栏】【固定区域】【UOS AI】小U同学插件图标多次左键点击响应正常', async ({ device, agent, uos }) => {
      // 步骤 1: 点击一次任务栏上UOS AI插件图标
      await agent.aiTap("任务栏上最右侧的UOS AI插件图标", { deepThink: true });
      ...
      // 步骤 2: 再次点击任务栏上UOS AI插件图标
      await agent.aiTap("任务栏上最右侧的UOS AI插件图标", { deepThink: true });

      //检查: 最小化UOS AI应用窗口
      await agent.aiAssert("小U同学应用窗口已消失");

      // 步骤 3: 继续点击UOS AI插件图标
      await agent.aiTap("任务栏上最右侧的UOS AI插件图标", { deepThink: true });
      await agent.aiWaitFor("小U同学应用窗口已显示");

      //检查: 按上述预期1，2响应，不会出现崩溃现象
      await agent.aiAssert("小U同学应用窗口正常响应，未出现崩溃现象");

    }, { timeout: 1200000, tags: ["2004529", "level3"] });
