
/**
 * 用例 PMSID: 1970503
 * 用例标题: 【控制中心】【蓝牙和其他设备】【键盘】【快捷键】打开窗口菜单快捷键冲突
 * 生成时间: 2026-01-29 13:27:57
 * 用例编写人:UT005571(王艺桥)
 */

describe('1970503-【控制中心】【蓝牙和其他设备】【键盘】【快捷键】打开窗口菜单快捷键冲突', () => {
  beforeAll(async ({ device, uos, agent }) => {
    console.log('1. beforeAll: 初始化测试套件');
    await uos.showDesktop();
  });

  beforeEach(async ({ device, agent, system }) => {
    console.log('2. beforeEach: 每个测试前的准备');
    // 恢复默认快捷键
    await system.exec(`dbus-send --session --print-reply --dest=com.deepin.daemon.Keybinding /com/deepin/daemon/Keybinding com.deepin.daemon.Keybinding.Reset`);
  });

  test('1970503-【控制中心】【蓝牙和其他设备】【键盘】【快捷键】打开窗口菜单快捷键冲突', async ({ device, agent, uos }) => {
    // 步骤 1: 打开控制中心-蓝牙和其他设备-键盘-快捷键
    await uos.openApp("控制中心",{maximizeWindow: true});
    await agent.aiTap("左侧：蓝牙和其他设备");
    await agent.aiTap("键盘");
    await agent.aiTap("快捷键");
    // 修改已存在的快捷键，例如显示工作区：Super+S
    await agent.aiScroll('蓝牙和其他设备/快捷键/键盘下方区域',{direction:'down',distance:10});
    await agent.aiTap("打开窗口菜单右侧:Alt space",{ deepThink: true });
    await device.pressKey("Super", "S");
    // 检查: 文案显示快捷键冲突，点击取消或者替换
    await agent.aiAssert("文案显示：此快捷键与[显示工作区]冲突，点击取消或替换");

    // 步骤 2: 快捷键冲突，点击取消按钮
    await agent.aiTap("取消",{ deepThink: true });
    // 检查: 打开窗口菜单快捷键还是Alt space
    await agent.aiAssert("打开窗口菜单右侧显示:Alt space");

    // 步骤 3: 快捷键冲突，点击替换按钮
    await agent.aiTap("打开窗口菜单右侧:Alt space",{ deepThink: true });
    await device.pressKey("Super", "S");
    await agent.aiAssert("文案显示：此快捷键与[显示工作区]冲突，点击取消或替换");
    await agent.aiTap("替换",{ deepThink: true });

    // 检查: 打开窗口菜单快捷键还是Alt space
    await agent.aiAssert("打开窗口菜单右侧显示:Super S");

    // 步骤 4: 检查显示工作区快捷键显示
    await agent.aiScroll('蓝牙和其他设备/快捷键/键盘下方区域',{direction:'up',distance:10});

    // 检查: 显示工作区快捷键显示无
    await agent.aiAssert("显示工作区右侧显示:无");

    // 步骤 5: 使用快捷键Super+S
    await device.pressKey("Super", "S");

    // 检查: 控制中心左上角显示菜单窗口
    await agent.aiAssert("应用窗口菜单已唤出");
    await agent.aiAssert("窗口左上角窗口显示：最小化，还原，移动等");

    // 关闭应用菜单
    await agent.aiTap("蓝牙和其他设备/快捷键/键盘下方区域");

    // 步骤 6: 恢复默认按钮
    await agent.aiTap("恢复默认");
    await new Promise(resolve => setTimeout(resolve,1000));

    // 检查: 打开窗口菜单和显示工作区恢复默认快捷键
    await agent.aiAssert("显示工作区右侧显示：Super S");
    await agent.aiScroll('蓝牙和其他设备/快捷键/键盘下方区域',{direction:'down',distance:10});
    await agent.aiAssert("打开窗口菜单右侧显示:Alt space");
    
  }, { timeout: 600000, tags: ['1970503', 'level3'] });

  afterEach(async ({ device,uos }) => {
    console.log('4. afterEach: 每个测试后的清理');
    await device.pressKey("Super", "Down");
    await uos.closeCurrentWindow();
  });

  afterAll(async ({ uos, agent, device, system }) => {
    console.log('5. afterAll: 清理测试套件');
  });
});
