/**
 * 用例 PMSID: 1975163
 * 用例标题:【控制中心】【账户】管理员重设其它账户的密码后保存的密码默认为国密算法sm3
 * 生成时间: 2026-03-30
 * 用例编写人:UT005044(王亮)
 */

describe('1975163-【控制中心】【账户】管理员重设其它账户的密码后保存的密码默认为国密算法sm3', () => {
    const testData = {
    ...
    test('1975163-【控制中心】【账户】管理员重设其它账户的密码后保存的密码默认为国密算法sm3', async ({ device, agent, env, uos, system }) => {
        // 步骤1：获取加密算法dconfig配置
        ...
        for (let i = 0; i < testData.username.length; i++) {
            ...
            // 步骤 5: 重设账户的密码
            await agent.aiTap(`点击其它账户下方列表的${testData.username[i]}`, { deepThink: true });              
            await agent.aiTap("点击登录方式下方的密码", { deepThink: true });     
            await agent.aiTap("点击重设密码", { deepThink: true });
            await agent.aiWaitFor("重设密码弹框");
            await agent.aiTap("点击新密码右侧的输入框", { deepThink: true });   
            await device.typeText(testData.resetpwd[i]);
            await agent.aiTap("点击重复密码右侧的输入框", { deepThink: true });  
            await device.typeText(testData.resetpwd[i]);   
            await agent.aiTap("点击修改密码按钮", { deepThink: true }); 
            if ( await agent.aiBoolean("是否有修改用户数据需要认证的弹框", { timeout: 3000 })) {
                await device.typeText(env.testPassword);
                await agent.aiTap("点击确定按钮", { deepThink: true });  
            }
            await new Promise(resolve => setTimeout(resolve, 2000)); // 延迟2秒（避免过快操作）
            console.log(`${testData.username[i]}用户重设密码完成！`);

             // 步骤6：返回到账户默认页面           
            await agent.aiTap("点击导航栏处的账户", { deepThink: true }); 

            // 步骤7：读取/etc/shadow文件中对应账户的加密算法数据
    ...
    }, { timeout: 700000, tags: ["1975163", "level2", "smoke" ] });
