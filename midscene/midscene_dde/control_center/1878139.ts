...
/**
 * 用例 PMSID: 1878139
 * 用例标题: 【控制中心】【系统更新】更新设置页面，开启自动下载开关，闲时下载时间修改后重启再次查看闲时下载时间，是上一次修改的时间
 * 生成时间: 2026-02-03 10:05:06
 * 用例编写人: UT001924（李鹤）
 */

describe('1878139-【控制中心】【系统更新】更新设置页面，开启自动下载开关，闲时下载时间修改后重启再次查看闲时下载时间，是上一次修改的时间', () => {
  ...
  test('1878139-【控制中心】【系统更新】更新设置页面，开启自动下载开关，闲时下载时间修改后重启再次查看闲时下载时间，是上一次修改的时间', async ({ device, agent, uos, system }) => {
    // 定义一个打开控制中心进入系统更新设置页面并展开高级设置的函数方便后面重复调用
    ...
    // 修改闲时下载时间
    await agent.aiTap("'开始时间'输入框右边黑色圆形按钮向上箭头", { deepThink: true});
    await agent.aiTap("'结束时间'输入框右边黑色圆形按钮向上箭头", { deepThink: true});
    // 等待修改生效
    await agent.aiWaitFor("'开始时间'是18:00'");
    await agent.aiWaitFor("'结束时间'是21:00'");
    // 关闭控制中心后再次进入更新设置页面展开高级设置页面
    await device.pressKey("super", "Down");
    await device.pressKey("alt", "F4");
    // 调用进入更新设置页面并展开高级设置函数
    await openAdvancedSettings();
    // 断言闲时下载时间是上一次修改的时间
    await agent.aiAssert("'开始时间'是18:00'");
    await agent.aiAssert("'结束时间'是21:00'");
  }, { timeout: 600000, tags: ['1878139', 'level3'] });
  ...
  afterEach(async ({ device, system, agent }) => {
    ...
    // 等待修改生效
    await agent.aiWaitFor("'开始时间'是17:00'");
    await agent.aiWaitFor("'结束时间'是20:00'");
    // 关闭自动下载开关恢复默认状态
