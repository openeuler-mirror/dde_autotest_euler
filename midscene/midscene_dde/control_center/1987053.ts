/**
 * 用例 PMSID: 1987053
 * 用例标题: 【控制中心】【系统更新】【公网】传递优化配置项的下载限速配置项速度输入框不允许非数字输入
 * 生成时间: 2026-06-01 10:00:00
 * 用例编写人:UT000327(秦家喜)
 */

describe('1987053-【控制中心】【系统更新】【公网】传递优化配置项的下载限速配置项速度输入框不允许非数字输入', () => {
  ...
  test('1987053-下载限速输入框不允许非数字输入', async ({ device, agent, uos }) => {
    ...
    await device.typeText("abc");
    await agent.aiAssert("传递优化-下载限速输入框无法输入非数字字符，输入框中不显示abc");

    await agent.aiTap("'传递优化-下载限速'区域输入框");
    await device.pressKey("ctrl", "a");
    await device.typeText("!@#");
    await agent.aiAssert("传递优化-下载限速输入框无法输入特殊字符，输入框中不显示!@#");  
  }, { timeout: 600000, tags: ['1987053', 'level4'] });
