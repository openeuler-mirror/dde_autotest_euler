/**
 * 用例 PMSID: 1995253
 * 用例标题: 【控制中心】【系统更新】【公网】传递优化配置项的下载限速配置项速度输入框，输入支持的最大值和最小值，正常输入
 * 生成时间: 2026-06-01 10:00:00
 * 用例编写人:UT000327(秦家喜)
 */

describe('1995253-【控制中心】【系统更新】【公网】传递优化配置项的下载限速配置项速度输入框，输入支持的最大值和最小值，正常输入', () => {
  ...
  test('1995253-下载限速输入最大值和最小值正常输入', async ({ device, agent, uos }) => {
    ...
    await agent.aiAssert("传递优化-下载限速输入框显示10");

    await agent.aiTap("'传递优化-下载限速'区域输入框");
    await device.pressKey("ctrl", "a");
    await device.typeText("999999");
    await device.pressKey("Tab");
    await agent.aiAssert("传递优化-下载限速输入框显示999999");
  }, { timeout: 600000, tags: ['1995253', 'level3'] });
