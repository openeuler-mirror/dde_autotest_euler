...
/**
 * 用例 PMSID: 1878407
 * 用例标题: 【控制中心】【系统更新】更新设置页面点击高级设置展开控件后，重启控制中心再次进入更新设置页面，高级设置项是收起状态
 * 生成时间: 2026-01-21 10:02:31
 * 用例编写人: UT001924(李鹤)
 */

describe('1878407-【控制中心】【系统更新】更新设置页面点击高级设置展开控件后，重启控制中心再次进入更新设置页面，高级设置项是收起状态', () => {
  ...
  test('1878407-【控制中心】【系统更新】更新设置页面点击高级设置展开控件后，重启控制中心再次进入更新设置页面，高级设置项是收起状态', async ({ device, agent, uos }) => {
    // 定义一个进入系统更新设置页面的函数(包含启动控制中心)方便后面重复调用
    ...
    // 调用进入系统更新设置页面的函数(包含启动控制中心)进入系统更新设置页面
    await enterUpdateSettings();
    // 查看高级设置是否是收起状态
    await agent.aiAssert("'展开'文字可见");
  }, { timeout: 1200000, tags: ['1878407', 'level3'] });
