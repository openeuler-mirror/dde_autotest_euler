...
/**
 * 用例 PMSID: 1877799
 * 用例标题: 【控制中心】【系统更新】更新设置页面，下载限速开关开启，输入框输入10以下的数字，提示错误信息
 * 生成时间: 2026-02-04 13:20:19
 * 用例编写人: UT001924（李鹤）
 */

describe('1877799-【控制中心】【系统更新】更新设置页面，下载限速开关开启，输入框输入10以下的数字，提示错误信息', () => {
  ...
  test('1877799-【控制中心】【系统更新】更新设置页面，下载限速开关开启，输入框输入10以下的数字，提示错误信息', async ({ device, agent, uos }) => {
    // 打开控制中心并最大化
    ...
    // 选中下载输入框，全选后输入9，焦点离开输入框
    await agent.aiTap("限速设置'区域输入框");
    await device.pressKey("ctrl", "a");
    await device.typeText("9");
    await device.pressKey("tab");
    // 断言输入报错
    await agent.aiAssert("输入框显示1024,提示错误信息：只允许输入10-999999,输入框底色为红色");
  }, { timeout: 600000, tags: ['1877799', 'level4'] });
