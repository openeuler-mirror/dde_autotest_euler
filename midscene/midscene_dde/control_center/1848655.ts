/**
 * 用例 PMSID: 1848655
 * 用例标题:统信桌面操作系统 V25(#493) - 【桌面】【登录】重启-进入用户登录界面
 * 生成时间: 2026-05-13
 * 用例编写人: UT002485(卢燕)
 */

describe('1848655-统信桌面操作系统 V25(#493) - 【桌面】【登录】重启-进入用户登录界面', () => {
    ...
    test('1848655-重启-进入用户登录界面', async ({ device, agent, uos, env }) => {
        // env中获取测试账号密码
        ...
        const isInitialized = await agent.aiBoolean('显示以上程序阻止重启');
        if (isInitialized) {
            await agent.aiTap('重启按钮');
        } 

        // 预期: 重启后进入用户登录界面
        await agent.aiWaitFor('登录界面已显示', { timeoutMs: 120000 });
        ...
        await agent.aiWaitFor('桌面已显示', { timeoutMs: 60000 });
    }, { timeout: 300000, tags: ['1848655','level1','smoke','remote'] });
