/**
 * 用例 PMSID: 1987031
 * 用例标题: 【控制中心】【系统更新】【公网】未修改过传递优化配置项的上传限速数值，打开开关，默认展示10240KB/s
...
describe('1987031-【控制中心】【系统更新】【公网】未修改过传递优化配置项的上传限速数值，打开开关，默认展示10240KB/s', () => {
  ...
  test('1987031-未修改过上传限速数值打开开关默认展示10240', async ({ device, agent, uos }) => {
    ...
    await agent.aiAssert("传递优化-上传限速输入框默认显示10240KB/s");
  }, { timeout: 600000, tags: ['1987031', 'level3'] });

  afterEach(async ({ device, agent }) => {
    console.log('4. afterEach: 每个测试后的清理');
    await agent.aiTap("'传递优化-上传限速'区域复选框");
    await agent.aiWaitFor("'传递优化-上传限速'区域输入框置灰不可编辑");
    await agent.aiTap("'传递优化-下载限速'区域复选框");
    await agent.aiWaitFor("'传递优化-下载限速'区域输入框置灰不可编辑");
