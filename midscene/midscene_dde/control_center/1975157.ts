/**
 * 用例 PMSID: 1975157
 * 用例标题:【控制中心】【账户】新建用户保存的密码默认为国密算法sm3
 * 生成时间: 2026-03-20
 * 用例编写人:UT005044(王亮)
 */

describe('1975157-【控制中心】【账户】新建用户保存的密码默认为国密算法sm3', () => {
    const testData = {
    ...
    test('1975157-【控制中心】【账户】新建用户保存的密码默认为国密算法sm3', async ({ device, agent, env, uos, system }) => {
        // 步骤1：获取加密算法dconfig配置
        ...
        for (let i = 0; i < testData.username.length; i++) {
            console.log(`正在创建用户: ${testData.username[i]}`);
            // 步骤 4: 创建账户
            ...
            // 步骤5：读取/etc/shadow文件中对应账户的加密算法数据
    ...
    }, { timeout: 600000, tags: ["1975157", "level2", "smoke" ] });
