/**
 * 用例 PMSID: 1506859
 * 用例标题: 【控制中心】【网络】【飞行模式】飞行模式关闭后WiFi自动回连
 * 生成时间: 2025/12/23 18:51
 * 用例编写人: UT0029989(熊林辉)
 */

describe('1506859-【控制中心】【网络】【飞行模式】飞行模式关闭后WiFi自动回连', () => {
    ...
    test('1506859-【控制中心】【网络】【飞行模式】飞行模式关闭后WiFi自动回连', async ({ device, agent, uos, system}) => {
	 // ========== 新增：无线网卡存在性判断 ==========
      ...
      // 步骤 5: 点击任务栏快捷设置，打开飞行模式
      await agent.aiTap("任务栏的'音量图标'", { deepThink: true });
      await agent.aiWaitFor("桌面右下区域'飞行模式'文字可见");
      await agent.aiTap("'飞行模式图标'", { deepThink: true });
      await agent.aiAssert("无线网络显示关闭字样");

      // 步骤 5: 关闭飞行模式，WiFi回连检查
      await agent.aiTap("'飞行模式图标'", { deepThink: true });
      await agent.aiWaitFor("蓝牙显示未连接字样");
      await agent.aiAssert("无线网络显示uniontech-m字样")

    }, { timeout: 300000, tags: ["1506859","level2","smoke", "Laptop"] });
