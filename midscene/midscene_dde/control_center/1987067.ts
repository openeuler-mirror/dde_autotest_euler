/**
 * 用例 PMSID: 1987067
 * 用例标题: 【控制中心】【系统更新】【公网】改变传递优化配置项的下载限速配置项复选框状态，切换模块，展示切换模块前的状态
 * 生成时间: 2026-06-01 10:00:00
 * 用例编写人:UT000327(秦家喜)
 */

describe('1987067-【控制中心】【系统更新】【公网】改变传递优化配置项的下载限速配置项复选框状态，切换模块，展示切换模块前的状态', () => {
  ...
  test('1987067-改变下载限速复选框状态切换模块展示切换前状态', async ({ device, agent, uos }) => {
    ...
    await agent.aiAssert("传递优化-下载限速复选框为勾选状态");

    await agent.aiTap("系统", { deepThink: true });
    await agent.aiWaitFor("'显示'文字可见");
    await agent.aiTap("系统更新", { deepThink: true });
    await agent.aiWaitFor("'有可用的更新'或'重新检查更新'文字可见", {timeoutMs: 30000});
    await agent.aiTap("更新设置", { deepThink: true });
    await agent.aiWaitFor("'更新类型'文字可见");
    ...
    await agent.aiAssert("传递优化-下载限速复选框为勾选状态"); 
  }, { timeout: 1200000, tags: ['1987067', 'level3'] });
