/**
 * 用例 PMSID: 1801229
 * 用例标题: 【控制中心】【设备】【键盘】【快捷键】快捷键搜索框输入正常字符，查看匹配项显示
 * 生成时间: 2026-09-11
 * 用例编写人:UT005571(王艺桥)
 */

describe('1801229-【控制中心】【设备】【键盘】【快捷键】快捷键搜索框输入正常字符，查看匹配项显示', () => {
  ...
  test('1801229-【控制中心】【设备】【键盘】【快捷键】快捷键搜索框输入正常字符，查看匹配项显示', async ({ device, agent, uos }) => {
    // 打开控制中心，进入键盘-快捷键页面
    ...
    // 步骤 1: 点击快捷键搜索框
    await agent.aiTap("快捷键设置页面顶部的搜索框（内有搜索图标）");
    // 步骤 2: 输入字符 p
    await device.typeText("p");
    await new Promise(resolve => setTimeout(resolve, 800));
    // 检查：显示匹配结果（不区分大小写）
    await agent.aiAssert("界面展示快捷键搜索结果列表，结果中包含名称含字母 p（大小写均可）的快捷键条目", { timeoutMs: 10000 });
    // 恢复：清空搜索框并退出搜索状态
    await device.pressKey("Escape");
  }, { timeout: 600000, tags: ['1801229', 'level3'] });
