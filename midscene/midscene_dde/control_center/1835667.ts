/**
 * 用例 PMSID: 1835667
 * 用例标题: 【控制中心】【电源管理】【通用】设置定时关机重复次数为自定义，保存默认的重复时间
 * 生成时间: 2026-2-9 15:11:10
 * 用例编写人:UT000511(肖海燕)
 */

describe('1835667-【控制中心】【电源管理】【通用】设置定时关机重复次数为自定义，保存默认的重复时间', () => {
    ...
    test('1835667-【控制中心】【电源管理】【通用】设置定时关机重复次数为自定义，保存默认的重复时间', async ({ device, agent, uos }) => {
      //步骤 1: 打开控制中心并最大化
      ...
      //步骤 5: 点击保存按钮,检查重复设置项显示为自定义,重复设置项下方显示重复时间：星期一至星期五，右侧显示编辑按钮
      await agent.aiTap('点击保存');
      await agent.aiAssert('重复设置项显示为自定义,重复设置项下方显示重复时间：星期一至星期五，星期五文案后面显示铅笔图标形状的按钮');

    }, { timeout: 600000, tags: ["1835667","level3"] });
