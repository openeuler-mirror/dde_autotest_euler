/**
 * 用例 PMSID: 1987047
 * 用例标题: 【控制中心】【系统更新】【公网】清空传递优化配置项的上传限速配置项速度输入框的值，焦点离开输入框，输入框恢复上次输入框中的数值
 * 生成时间: 2026-06-01 10:00:00
 * 用例编写人:UT000327(秦家喜)
 */

describe('1987047-【控制中心】【系统更新】【公网】清空传递优化配置项的上传限速配置项速度输入框的值，焦点离开输入框，输入框恢复上次输入框中的数值', () => {
  ...
  test('1987047-清空上传限速输入框焦点离开恢复原值', async ({ device, agent, uos }) => {
    ...
    await agent.aiAssert("传递优化-上传限速输入框为空");
    await device.pressKey("Tab");
    await agent.aiAssert("传递优化-上传限速输入框恢复上次输入框中的数值");
  }, { timeout: 600000, tags: ['1987047', 'level4'] });
