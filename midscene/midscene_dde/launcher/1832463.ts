/**
 * 用例 PMSID: 1832463
 * 用例标题: 【启动器】【窗口模式】【搜索框】搜索无结果时，缺省图检查
 * 生成时间: 2026/1/30 16:31
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832463-【启动器】【窗口模式】【搜索框】搜索无结果时，缺省图检查', () => {
    ...
    test('1832463-【启动器】【窗口模式】【搜索框】搜索无结果时，缺省图检查', async ({ device, agent, uos }) => {
      // 步骤 1: 打开启动器
      await uos.openLauncher();

      // 步骤 2: 搜索框输入特殊字符，如：*app
      await agent.aiTap("启动器底部中央的搜索输入框，呈矩形，内有放大镜图标");
      await device.typeText('*app');
      // 检查：搜索不到相应的结果，显示无搜索结果页面
      await agent.aiAssert("启动器中间/右侧区域展示：带有灰色放大镜图标和 “无搜索结果” 文字的搜索结果空白展示区");
    }, { timeout: 120000, tags: ["1832463", "level3"] });
