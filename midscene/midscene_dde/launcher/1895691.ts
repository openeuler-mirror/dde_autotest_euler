/**
 * 用例 PMSID: 1895691
 * 用例标题: 【启动器】【窗口模式】【搜索框】大写字母开头单词匹配
 * 生成时间: 2026/1/30 13:45
 * 用例编写人: UT002998(熊林辉)
 */

describe('1895691-【启动器】【窗口模式】【搜索框】大写字母开头单词匹配', () => {
    beforeAll(async ({ device, uos, agent}) => {
      console.log('1. beforeAll: 初始化测试套件');
    });

    beforeEach(async ({ agent, system, uos, device}) => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1895691-【启动器】【窗口模式】【搜索框】大写字母开头单词匹配', async ({ device, agent, uos}) => {
      // 步骤 1: 打开启动器
      await uos.openLauncher();

      // 步骤 2：搜索框输入Movie
      await device.typeText('Movie');
      // 检查：可搜索到影院应用
      await agent.aiAssert("启动器页面存在：所有应用");
      await agent.aiAssert("启动器页面存在：影院应用图标");
      ...
      // 步骤 4：搜索框输入UC
      await device.typeText('Mov');
      // 检查：可搜索到影院应用
      await agent.aiAssert("启动器页面存在：所有应用");
      await agent.aiAssert("启动器页面存在：影院应用图标");

    }, { timeout: 300000, tags: ["1895691", "level3"] });
