/**
 * 用例 PMSID: 1832577
 * 用例标题: 【启动器】【全屏模式】【键盘方向键】点击键盘下键，应用选择与翻页
 * 生成时间: 2026-04-29
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832577-【启动器】【全屏模式】【键盘方向键】点击键盘下键，应用选择与翻页', () => {
    ...
    test('1832577-【启动器】【全屏模式】【键盘方向键】点击键盘下键，应用选择与翻页', async ({ device, agent, uos }) => {
        // 步骤 1: 打开启动器，点击下键
        await uos.openLauncher();
        await agent.aiTap({
        ...
        // 步骤 2: 继续点击键盘下键
        await device.pressKey('Down');
        // 检查 2: 选中当前页的从上往下第二个应用，为select状态
        await agent.aiAssert('第二行的第一个应用图标呈select选中状态');

        // 步骤 3: 继续点击键盘下键
        await device.pressKey('Down');
        await agent.aiAssert('第三行的第一个应用图标呈select选中状态');

    }, { timeout: 600000, tags: ['1832577', 'level3'] });
