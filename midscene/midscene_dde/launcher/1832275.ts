/**
 * 用例 PMSID: 1832275
 * 用例标题: 【启动器】【窗口模式】【“按名称”分类】【应用列表键盘操作】tab键切换焦点至应用列表
 * 生成时间: 2026/03/20 15:30
 * 用例编写人: UT005044(王亮)
 */

describe('1832275-【启动器】【窗口模式】【“按名称”分类】【应用列表键盘操作】tab键切换焦点至应用列表', () => {
  ...
  test('1832275-【启动器】【窗口模式】【“按名称”分类】【应用列表键盘操作】tab键切换焦点至应用列表', async ({ device, agent, uos }) => {
    //步骤1：打开启动器
    await uos.openLauncher();
    //步骤2：打开桌面文件夹
    await agent.aiTap({
   ...
   // 步骤 5: 点击键盘TAB，切换焦点至字母表首字母
    await device.pressKey('TAB');
    await agent.aiAssert("字母表首字母A有被选中的效果");

   // 步骤 6: 点击键盘ESC，隐藏分类面板
    await device.pressKey('ESC');
    await agent.aiAssert("启动器界面存在字母分类：A、B、C、D，不存在G、H分类");

    
  }, { timeout: 500000, tags: ["1832275", "level3"] });
