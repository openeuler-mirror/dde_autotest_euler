/**
 * 用例 PMSID: 1832313
 * 用例标题: 【启动器】【窗口模式】【“按分类”分类】【鼠标操作】鼠标左键点击分类列表标题，快速切换到对应分类列表
 * 生成时间: 2025/12/23 09:40
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832313-【启动器】【窗口模式】【“按分类”分类】【鼠标操作】鼠标左键点击分类列表标题，快速切换到对应分类列表', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832313-【启动器】【窗口模式】【“按分类”分类】【鼠标操作】鼠标左键点击分类列表标题，快速切换到对应分类列表', async ({ device, agent, uos }) => {
      //步骤1：打开启动器
      await uos.openLauncher();
      //步骤2：打开桌面文件夹
      await agent.aiTap({
    ...
    // 步骤 4: 鼠标左键点击分类“网络应用”
      ...
      await agent.aiAssert("启动器界面存在分类：网络应用、音乐欣赏、视频播放、图形图像");

    // 步骤 5: 鼠标左键点击分类“视频播放”
      await agent.aiTap("视频播放");
      await agent.aiAssert('启动器窗口的左侧面板中，“视频播放”选项显示在顶部');

    }, { timeout: 300000, tags: ["1832313", "level2", "smoke"] });
