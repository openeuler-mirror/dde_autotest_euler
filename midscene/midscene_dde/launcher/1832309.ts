/**
 * 用例 PMSID: 1832309
 * 用例标题: 【启动器】【窗口模式】【“按分类”分类】【鼠标操作】鼠标hover到效果检查
 * 生成时间: 2026/03/18 15:10
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832309-【启动器】【窗口模式】【“按分类”分类】【鼠标操作】鼠标hover到效果检查', () => {
  ...
  test('1832309-【启动器】【窗口模式】【“按分类”分类】【鼠标操作】鼠标hover到效果检查', async ({ device, agent, uos }) => {
    //步骤1：打开启动器
    await uos.openLauncher();
    //步骤2：打开桌面文件夹
    await agent.aiTap({
    ...
    // 步骤 4: 鼠标hover到“网络应用”分类
    await agent.aiHover('网络应用');
    await agent.aiAssert("网络应用被选中，底部有灰色底色效果");
    // 步骤 5: 鼠标hover到应用上
    await agent.aiHover('浏览器');
    await agent.aiAssert("浏览器应用被选中，底部有灰色底色效果");

  }, { timeout: 300000, tags: ["1832309", "level3"] });
