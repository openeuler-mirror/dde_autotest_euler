/**
 * 用例 PMSID: 1832311
 * 用例标题: 【启动器】【窗口模式】【“按分类”分类】【鼠标操作】点击分类标题，显示分类列表
 * 生成时间: 2026/1/22 18:40
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832311-【启动器】【窗口模式】【“按分类”分类】【鼠标操作】点击分类标题，显示分类列表', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832311-【启动器】【窗口模式】【“按分类”分类】【鼠标操作】点击分类标题，显示分类列表', async ({ device, agent, uos }) => {
      //步骤1：打开启动器
      await uos.openLauncher();
    ...
    // 步骤 3: 鼠标左键点击“按分类”
      ...
      await agent.aiAssert("启动器界面存在分类：网络应用、音乐欣赏");
    ...
    // 步骤 5: 点击键盘下键
      ...
      await agent.aiAssert("选中了'音乐欣赏'");

    // 步骤 6: 点击键盘ESC
      await device.pressKey('ESC');

    }, { timeout: 300000, tags: ["1832311", "level2", "smoke"] });
