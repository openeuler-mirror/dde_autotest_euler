/**
 * 用例 PMSID: 1832551
 * 用例标题: 【启动器】【全屏模式】【翻页规则】启动器应用有两页，页签圆点热区检查
 * 生成时间: 2026/5/26
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832551-【启动器】【全屏模式】【翻页规则】启动器应用有两页，页签圆点热区检查', () => {
    ...
    test('1832551-【启动器】【全屏模式】【翻页规则】启动器应用有两页，页签圆点热区检查', async ({ device, agent, uos }) => {
        // 步骤 1: 打开启动器，切换到全屏模式
        await uos.openLauncher();
        await agent.aiTap({
        ...
        // 步骤 2: 检查页签圆点热区
        await agent.aiAssert('启动器界面顶部中央有2个圆点');

        // 步骤 3: 点击两圆点正中间偏左区域
        await agent.aiTap('启动器界面顶部两个圆点正中间偏左区域');
        await agent.aiWaitFor('启动器全屏模式已显示');
        ...
        // 步骤 4: 点击两圆点正中间偏右区域
        await agent.aiTap('启动器界面顶部,第二个圆点的右边一点点');
        await agent.aiWaitFor('启动器全屏模式已显示');

        // 检查2: 切换到第二页
        await agent.aiAssert('启动器界面顶部，页面指示点中，第一个点是灰色（未激活），第二个点是白色（激活）');

    }, { timeout: 600000, tags: ['1832551', 'level3'] });
