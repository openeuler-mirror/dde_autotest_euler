/**
 * 用例 PMSID: 1832527
 * 用例标题: 【启动器】【全屏模式】【展示规则】应用列表仅有自由排序模式
 * 生成时间: 2025/12/19 15:23
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832527-【启动器】【全屏模式】【展示规则】应用列表仅有自由排序模式', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832527-【启动器】【全屏模式】【展示规则】应用列表仅有自由排序模式', async ({ device, agent, uos }) => {
    ...
    // 如果3次都失败，抛出错误
      ...
      // 检查: 仅有自由排序模式
      await agent.aiAssert("启动器页面左上角不存在切换菜单");

    }, { timeout: 600000, tags: ["1832527", "level1", "smoke"] });
