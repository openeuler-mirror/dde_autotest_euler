/**
 * 用例 PMSID: 1832297
 * 用例标题: 【启动器】【窗口模式】【“按分类”分类】【排序规则】当系统无某类应用，则无该标签分类
 * 生成时间: 2026/03/18 15:30
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832297-【启动器】【窗口模式】【“按分类”分类】【排序规则】当系统无某类应用，则无该标签分类', () => {
  ...
  test('1832297-【启动器】【窗口模式】【“按分类”分类】【排序规则】当系统无某类应用，则无该标签分类', async ({ device, agent, uos }) => {
    //步骤1：打开启动器
    await uos.openLauncher();
    //步骤2：打开桌面文件夹
    await agent.aiTap({
    ...
    await agent.aiAssert("启动器界面不存在分类：社交沟通");
    // 步骤 5: 点击键盘ESC，隐藏分类面板
    await device.pressKey('ESC');
  }, { timeout: 300000, tags: ["1832297", "level1", "smoke"] });
