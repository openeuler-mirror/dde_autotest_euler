/**
 * 用例 PMSID: 1832573
 * 用例标题: 【启动器】【全屏模式】【翻页规则】当前在应用列表第二页，切换到窗口模式后再切到全屏模式，重新停留在首页
 * 生成时间: 2026/5/26
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832573-【启动器】【全屏模式】【翻页规则】当前在应用列表第二页，切换到窗口模式后再切到全屏模式，重新停留在首页', () => {
    ...
    test('1832573-【启动器】【全屏模式】【翻页规则】当前在应用列表第二页，切换到窗口模式后再切到全屏模式，重新停留在首页', async ({ device, agent, uos }) => {
        // 步骤 1: 打开启动器，切换到全屏模式
        await uos.openLauncher();
        await agent.aiTap({
        ...
        // 检查1: 滑动不再翻页，仍然在第一页
        await agent.aiAssert('启动器界面顶部，页面指示点中，第一个点是白色（激活）');
        await agent.aiAssert('启动器界面仍然显示第一页的应用列表');

        // 步骤 2: 向上继续滑动鼠标滚轮（尝试从第一页继续向上滑）
         //await agent.aiScroll('应用列表区域向上滚动', { direction: 'up', distance: 500 });
        //await agent.aiWaitFor('启动器全屏模式已显示');
        await agent.aiTap('启动器界面顶部的页面指示点，第二个灰色（未激活）的点');
        await agent.aiWaitFor('启动器全屏模式已显示');
     ...
     agent.aiTap({
        ...
        // 检查2: 重新停留在首页（第一个圆点应该是白色/激活状态）
        await agent.aiAssert('启动器界面顶部，页面指示点中，第一个点是白色（激活），第二个点是灰色（未激活）');

    }, { timeout: 600000, tags: ['1832573', 'level3'] });
