/**
 * 用例 PMSID: 1832171
 * 用例标题: 【启动器】【全屏模式】【搜索规则】盲打键盘输入搜索框
 * 生成时间: 2025/12/22 11:07
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832171-【启动器】【全屏模式】【搜索规则】盲打键盘输入搜索框', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832171-【启动器】【全屏模式】【搜索规则】盲打键盘输入搜索框', async ({ device, agent, uos }) => {
      // 步骤 1: 打开启动器
      await uos.openLauncher();
      ...
      await agent.aiAssert("启动器页面存在：界面顶部中央有2个圆点，底部中央有一个带放大镜图标的搜索框,右上角有一个四角星图标");

      // 步骤 3: 鼠标没有点击搜索框时敲键盘任意键,如app123
      await device.typeText("app123")
      await agent.aiWaitFor("底部中央的搜索输入框，呈矩形，内有放大镜图标和app123");

      // 检查：实时响应搜索并显示结果
      await agent.aiAssert("启动器界面不显示应用图标")
      await agent.aiAssert("启动器界面显示文案：无搜索结果")

     // 步骤 4: 减少搜索字符，鼠标没有点击搜索框时敲键盘任意键,如app
      await agent.aiTap("搜索框的x");
      await agent.aiWaitFor("底部中央的搜索输入框，呈矩形，内有放大镜图标和空白输入区域");
      await agent.aiAssert("启动器页面存在：界面顶部中央有2个圆点，底部中央有一个带放大镜图标的搜索框,右上角有一个四角星图标");
      await device.typeText("app");

     // 检查：自动获取到键盘，键值输入到搜索框，实时响应搜索并显示结果
      await agent.aiWaitFor("底部中央的搜索输入框，呈矩形，内有放大镜图标和app");
      await agent.aiAssert("启动器界面显示唯一应用：应用商店")

    }, { timeout: 300000, tags: ["1832171", "level2", "smoke"] });
