/**
 * 用例 PMSID: 1832489
 * 用例标题: 【启动器】【窗口模式】唤出/关闭启动器
 * 生成时间: 2025/12/19 14:51
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832489-【启动器】【窗口模式】唤出/关闭启动器', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832489-【启动器】【窗口模式】唤出/关闭启动器', async ({ device, agent, uos }) => {
      // 步骤 1: 打开启动器
      await uos.openLauncher();
      await agent.aiAssert("界面存在：我的常用");

      // 步骤 2: 关闭启动器
      await uos.device.pressKey('ESC');
    ...
    }, { timeout: 600000, tags: ["1832489", "level2", "smoke"] });
