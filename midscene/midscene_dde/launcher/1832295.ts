/**
 * 用例 PMSID: 1832295
 * 用例标题: 【启动器】【窗口模式】【“按分类”分类】【排序规则】检查系统默认标签分类
 * 生成时间: 2025/12/24 10:31
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832295-【启动器】【窗口模式】【“按分类”分类】【排序规则】检查系统默认标签分类', () => {
  ...
  test('1832295-【启动器】【窗口模式】【“按分类”分类】【排序规则】检查系统默认标签分类', async ({ device, agent, uos }) => {
    //步骤1：打开启动器
    await uos.openLauncher();
    //步骤2：打开桌面文件夹
    await agent.aiTap({
    ...
    // 步骤 6: 点击键盘ESC，隐藏分类面板
    await device.pressKey('ESC');
  }, { timeout: 300000, tags: ["1832295", "level1", "smoke"] });
