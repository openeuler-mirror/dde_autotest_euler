/**
 * 用例 PMSID: 1832575
 * 用例标题: 【启动器】【全屏模式】【键盘方向键】点击键盘上键，应用选择与翻页
 * 生成时间: 2026-04-29
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832575-【启动器】【全屏模式】【键盘方向键】点击键盘上键，应用选择与翻页', () => {
    ...
    test('1832575-【启动器】【全屏模式】【键盘方向键】点击键盘上键，应用选择与翻页', async ({ device, agent, uos }) => {
        // 步骤 1: 打开启动器，点击上键
        await uos.openLauncher();
        await agent.aiTap({
        ...
        console.log('第一次按键盘上键');
        await device.pressKey('Up');
        await agent.aiWaitFor('第一个应用被选中，呈select状态');
        // 检查 1: 选中当前页的第一个应用，为select状态
        await agent.aiAssert('第一个应用图标呈select选中状态');

        // 步骤 2: 继续点击键盘上键
        console.log('第二次按键盘上键');
        await device.pressKey('Up');
        await agent.aiWaitFor('第一个应用图标没有select选中状态');
        // 检查 2: 焦点切换到搜索框
        await agent.aiAssert('底部搜索框获得焦点，有select选择状态，搜索框不展示搜索文案');

        // 步骤 3: 再次点击键盘上键
        console.log('第三次按键盘上键');
        await device.pressKey('Up');
        // 检查 3: 切换至上次选中的应用
         await agent.aiWaitFor('第一个应用被选中，呈select状态');
         await agent.aiAssert('第一个应用图标呈select选中状态');

    }, { timeout: 600000, tags: ['1832575', 'level3'] });
