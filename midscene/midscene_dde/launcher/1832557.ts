/**
 * 用例 PMSID: 1832557
 * 用例标题: 【启动器】【全屏模式】【翻页规则】鼠标滚轮操作，翻到最后一页无法继续向下滑动
 * 生成时间: 2026/5/26
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832557-【启动器】【全屏模式】【翻页规则】鼠标滚轮操作，翻到最后一页无法继续向下滑动', () => {
    ...
    test('1832557-【启动器】【全屏模式】【翻页规则】鼠标滚轮操作，翻到最后一页无法继续向下滑动', async ({ device, agent, uos }) => {
        // 步骤 1: 打开启动器，切换到全屏模式
        await uos.openLauncher();
        await agent.aiTap({
        ...
        // 步骤 2: 使用鼠标滚轮切换到最后一页（多次向下滑动）
        await agent.aiScroll('应用列表区域向下滑动', { direction: 'down', distance: 500 });
        await agent.aiWaitFor('启动器全屏模式已显示');

        // 检查1: 已切换到最后一页
        await agent.aiAssert('启动器界面顶部，页面指示点中，最后一个点是白色（激活）');

        // 步骤 3: 向下继续滑动鼠标滚轮（尝试从最后一页继续向下滑）
        await agent.aiScroll('应用列表区域向下滑动', { direction: 'down', distance: 500 });
        await agent.aiWaitFor('启动器全屏模式已显示');

        // 检查2: 滑动不再翻页，仍然在最后一页
        await agent.aiAssert('启动器界面顶部，页面指示点中，最后一个点是白色（激活）');
        await agent.aiAssert('启动器界面仍然显示最后一页的应用列表');

    }, { timeout: 600000, tags: ['1832557', 'level3'] });
