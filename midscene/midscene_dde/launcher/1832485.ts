/**
 * 用例 PMSID: 1832485
 * 用例标题: 【启动器】【窗口模式】窗口按分类和全屏模式来回切换
 * 生成时间: 2026/03/20 14:12
 * 用例编写人: UT005044(王亮)
 */

describe('1832485-【启动器】【窗口模式】窗口按分类和全屏模式来回切换', () => {
  ...
  test('1832485-【启动器】【窗口模式】窗口按分类和全屏模式来回切换', async ({ device, agent, uos }) => {
    //步骤1：打开启动器
    await uos.openLauncher();
    //步骤2：打开桌面文件夹
    await agent.aiTap({
  ...
  }, { timeout: 500000, tags: ["1832485", "level3"] });
