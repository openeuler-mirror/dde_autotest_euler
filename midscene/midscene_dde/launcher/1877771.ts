/**
 * 用例 PMSID: 1877771
 * 用例标题: 【启动器】【全屏模式】打开启动器全屏模式，鼠标点击任务栏等非启动器位置，启动器自动关闭
 * 生成时间: 2026/5/26
 * 用例编写人: UT002998(熊林辉)
 */

describe('1877771-【启动器】【全屏模式】打开启动器全屏模式，鼠标点击任务栏等非启动器位置，启动器自动关闭', () => {
    ...
    test('1877771-【启动器】【全屏模式】打开启动器全屏模式，鼠标点击任务栏等非启动器位置，启动器自动关闭', async ({ device, agent, uos }) => {
        // 步骤 1: 打开启动器全屏模式
        await uos.openLauncher();
        await agent.aiTap({
        ...
        await agent.aiAssert('启动器页面存在：界面顶部中央有2个圆点');
    ...
    }, { timeout: 600000, tags: ['1877771', 'level3'] });
