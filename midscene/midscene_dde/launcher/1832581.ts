/**
 * 用例 PMSID: 1832581
 * 用例标题: 【启动器】【全屏模式】【键盘方向键】点击键盘右键，应用选择与翻页
 * 生成时间: 2026-04-29
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832581-【启动器】【全屏模式】【键盘方向键】点击键盘右键，应用选择与翻页', () => {
    ...
    test('1832581-【启动器】【全屏模式】【键盘方向键】点击键盘右键，应用选择与翻页', async ({ device, agent, uos }) => {
        // 步骤 1: 打开启动器，点击键盘右键
        await uos.openLauncher();
        await agent.aiTap({
       ...
       // await agent.aiWaitFor('第一个应用被选中，呈select状态');
        ...
        // 步骤 2: 继续点击键盘右键
        await device.pressKey('Right');
        // await agent.aiWaitFor('依次横向第二个应用呈select状态');

        // 检查 2: 依次横向选择应用，直到切换到下一页
        await agent.aiAssert('依次横向第二个应用呈select状态');

    }, { timeout: 600000, tags: ['1832581', 'level3'] });
