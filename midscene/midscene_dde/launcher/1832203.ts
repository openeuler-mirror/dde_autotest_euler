/**
 * 用例 PMSID: 1832203
 * 用例标题: 【启动器】【全屏模式】【键盘交互】键盘super键功能验证
 * 生成时间: 2025/12/19 16:36
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832203-【启动器】【全屏模式】【键盘交互】键盘super键功能验证', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832203-【启动器】【全屏模式】【键盘交互】键盘super键功能验证', async ({ device, agent, uos }) => {
    ...
    // 如果3次都失败，抛出错误
      ...
      await agent.aiAssert("启动器页面展示浏览器应用、搜索输入框");

      // 步骤 3: 点击键盘super，关闭启动器全屏模式
      await agent.device.pressKey('Super');
      await agent.aiAssert("桌面不展示启动器窗口");

      // 步骤 4: 再次点击键盘super，打开启动器全屏模式
      await device.pressKey('Super');
      await agent.aiWaitFor("启动器页面已显示");
      await agent.aiAssert("启动器页面展示浏览器应用、搜索输入框");

      // 步骤 5: 再次点击键盘super，关闭启动器全屏模式
      await agent.device.pressKey('Super');
      await agent.aiAssert("桌面不展示启动器窗口");

    }, { timeout: 600000, tags: ["1832203", "level2", "smoke"] });
