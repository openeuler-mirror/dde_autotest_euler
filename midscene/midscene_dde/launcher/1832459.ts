/**
 * 用例 PMSID: 1832459
 * 用例标题: 【启动器】【窗口模式】【搜索框】盲打键盘输入搜索
 * 生成时间: 2026/1/22 11:31
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832459-【启动器】【窗口模式】【搜索框】盲打键盘输入搜索', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832459-【启动器】【窗口模式】【搜索框】盲打键盘输入搜索', async ({ device, agent, uos }) => {
      // 步骤 1: 打开启动器
      await uos.openLauncher();
      ...
      // 步骤 3: 鼠标没有点击搜索框时敲键盘任意键,如工 具
      await agent.aiTap("启动器搜索框的工具文案右侧的x");
      await agent.aiTap("启动器底部中央的搜索输入框，呈矩形，内有放大镜图标");
    ...
    }, { timeout: 120000, tags: ["1832459", "level2", "smoke"] });
