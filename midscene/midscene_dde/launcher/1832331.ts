/**
 * 用例 PMSID: 1832331
 * 用例标题: 【启动器】【窗口模式】【“按分类”分类】【键盘操作】进入分类列表，点击Esc退出
 * 生成时间: 2026/03/18 14:12
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832331-【启动器】【窗口模式】【“按分类”分类】【键盘操作】进入分类列表，点击Esc退出', () => {
  ...
  test('1832331-【启动器】【窗口模式】【“按分类”分类】【键盘操作】进入分类列表，点击Esc退出', async ({ device, agent, uos }) => {
    //步骤1：打开启动器
    await uos.openLauncher();
    //步骤2：打开桌面文件夹
    await agent.aiTap({
    ...
    // 步骤 6: 点击键盘ESC，隐藏分类面板
    await device.pressKey('ESC');
    await agent.aiAssert("启动器界面不存在分类：系统管理");
    
  }, { timeout: 300000, tags: ["1832331", "level3"] });
