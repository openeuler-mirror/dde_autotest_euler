/**
 * 用例 PMSID: 1832571
 * 用例标题: 【启动器】【全屏模式】【翻页规则】当前在应用列表第二页，关闭启动器后再次进入，仍然为第二页
 * 生成时间: 2026/5/26
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832571-【启动器】【全屏模式】【翻页规则】当前在应用列表第二页，关闭启动器后再次进入，仍然为第二页', () => {
    ...
    test('1832571-【启动器】【全屏模式】【翻页规则】当前在应用列表第二页，关闭启动器后再次进入，仍然为第二页', async ({ device, agent, uos }) => {
        // 步骤 1: 打开启动器，切换到全屏模式
        await uos.openLauncher();
        await agent.aiTap({
    ...
    }, { timeout: 600000, tags: ['1832571', 'level3'] });
