/**
 * 用例 PMSID: 1832555
 * 用例标题: 【启动器】【全屏模式】【翻页规则】鼠标滚轮操作，向上滑动
 * 生成时间: 2026-04-29
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832555-【启动器】【全屏模式】【翻页规则】鼠标滚轮操作，向上滑动', () => {
    ...
    test('1832555-【启动器】【全屏模式】【翻页规则】鼠标滚轮操作，向上滑动', async ({ device, agent, uos }) => {
        // 步骤 1: 打开启动器并切换到全屏模式
        await uos.openLauncher();
        await agent.aiTap({
        ...
        // 步骤 2: 全屏模式下包含多页应用，向下滑动鼠标滚轮

        await agent.aiScroll('应用列表区域向下滑动,不是任务栏', { direction: 'down', distance: 500 });
        // 检查: 向右翻页
        await agent.aiAssert('应用列表向右翻页显示');

        // 步骤 2: 再向上滑动鼠标滚轮
        await agent.aiScroll('应用列表区域向上滚动，不是任务栏', { direction: 'up', distance: 500 });
        // 检查: 向左翻页
        await agent.aiAssert('启动器界面顶部，页面指示点中，第一个点是白色（激活），第二个点是灰色（未激活）');

    }, { timeout: 600000, tags: ['1832555', 'level3'] });
