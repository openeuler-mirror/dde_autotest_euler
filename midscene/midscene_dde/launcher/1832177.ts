/**
 * 用例 PMSID: 1832177
 * 用例标题: 【启动器】【全屏模式】【搜索规则】搜索框输入中文
 * 生成时间: 2025/12/23 08:54
 * 用例编写人: UT002998(熊林辉)
 */

 describe('1832177-【启动器】【全屏模式】【搜索规则】搜索框输入中文', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832177-【启动器】【全屏模式】【搜索规则】搜索框输入中文', async ({ device, agent, uos }) => {
      // 步骤 1: 打开启动器
      await uos.openLauncher();
      ...
      // 步骤 3: 鼠标没有点击搜索框时敲键盘任意键,如工具
      await agent.aiTap("启动器底部中央的搜索输入框，呈矩形，内有放大镜图标");
      await device.typeText('工具');
      await agent.aiWaitFor("底部中央的搜索输入框，呈矩形，内有放大镜图标和工具");

      // 检查：实时响应搜索并显示结果
      await agent.aiAssert("启动器界面显示应用：日志收集工具、启动盘制作工具")

      // 步骤 4: 鼠标没有点击搜索框时敲键盘任意键,如工 具
      await agent.aiTap("搜索框的x");
      await agent.aiTap("启动器底部中央的搜索输入框，呈矩形，内有放大镜图标");
      await device.typeText('工 具');
      await agent.aiWaitFor("底部中央的搜索输入框，呈矩形，内有放大镜图标和工 具");

       // 检查：实时响应搜索并显示结果
      await agent.aiAssert("启动器界面显示应用：日志收集工具、启动盘制作工具")

    }, { timeout: 120000, tags: ["1832177", "level2", "smoke"] });
