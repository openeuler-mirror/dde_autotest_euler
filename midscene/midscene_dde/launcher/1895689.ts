/**
 * 用例 PMSID: 1895689
 * 用例标题: 【启动器】【窗口模式】【搜索框】支持小写字母中间匹配
 * 生成时间: 2026/1/30 14:18
 * 用例编写人: UT002998(熊林辉)
 */

describe('1895689-【启动器】【窗口模式】【搜索框】支持小写字母中间匹配', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1895689-【启动器】【窗口模式】【搜索框】支持小写字母中间匹配', async ({ device, agent, uos}) => {
      // 步骤 1: 打开启动器
      await uos.openLauncher();

      // 步骤 2：搜索框输入tore
      await device.typeText('tore');
      // 检查：可搜索到app store应用商店应用
      await agent.aiAssert("启动器页面存在：所有应用");
      await agent.aiAssert("启动器页面存在：应用商店图标");
    }, { timeout: 300000, tags: ["1895689", "level3"] });
