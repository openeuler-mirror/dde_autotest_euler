/**
 * 用例 PMSID: 1832549
 * 用例标题: 【启动器】【全屏模式】【翻页规则】启动器应用有两页，点击圆点翻页
 * 生成时间: 2026/5/26
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832549-【启动器】【全屏模式】【翻页规则】启动器应用有两页，点击圆点翻页', () => {
    ...
    test('1832549-【启动器】【全屏模式】【翻页规则】启动器应用有两页，点击圆点翻页', async ({ device, agent, uos }) => {
        // 步骤 1: 打开启动器，切换到全屏模式
        await uos.openLauncher();
        await agent.aiTap({
        ...
        // 步骤 2: 点击左边圆点
        await agent.aiTap('启动器界面顶部左边的圆点');
        await agent.aiWaitFor('启动器全屏模式已显示');
        ...
        // 步骤 3: 点击右边圆点
        await agent.aiTap('启动器界面顶部右边的圆点');
        await agent.aiWaitFor('启动器全屏模式已显示');

        // 检查3: 切换到第二页
        await agent.aiAssert('启动器界面顶部，页面指示点中，第一个点是灰色（未激活），第二个点是白色（激活）');

    }, { timeout: 600000, tags: ['1832549', 'level3'] });
