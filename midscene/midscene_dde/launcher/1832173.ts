/**
 * 用例 PMSID: 1832173
 * 用例标题: 【启动器】【全屏模式】【搜索规则】搜索结果界面UI和布局检查
 * 生成时间: 2026/2/9 11:26
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832173-【启动器】【全屏模式】【搜索规则】搜索结果界面UI和布局检查', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832173-【启动器】【全屏模式】【搜索规则】搜索结果界面UI和布局检查', async ({ device, agent, uos }) => {
    ...
    // 如果3次都失败，抛出错误
      ...
      // 步骤 3: 鼠标没有点击搜索框时敲键盘任意键,如 "a"
      await agent.aiTap("启动器底部中央的搜索输入框，呈矩形，内有放大镜图标");
      await device.typeText('a');
      await agent.aiWaitFor("底部中央的搜索输入框，呈矩形，内有放大镜图标和a");

      // 检查：实时响应搜索并显示结果
      await agent.aiAssert("启动器界面显示应用：界面以网格展示匹配的应用图标，一行最多显示了8个应用图标，右侧有垂直滚动条")
    }, { timeout: 300000, tags: ["1832173", "level3"] });
