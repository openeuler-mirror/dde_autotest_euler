/**
 * 用例 PMSID: 1832441
 * 用例标题: 【启动器】【窗口模式】【最近安装】安装完镜像，不展示“最近安装”菜单
 * 生成时间: 2025/12/22 09:53
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832441-【启动器】【窗口模式】【最近安装】安装完镜像，不展示“最近安装”菜单', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832441-【启动器】【窗口模式】【最近安装】安装完镜像，不展示“最近安装”菜单', async ({ device, agent, uos }) => {
      // 步骤1：打开启动器
      await uos.openLauncher();

      // 检查 2: 启动器窗口不展示“最近安装”菜单
      await agent.aiWaitFor("启动器页面已显示");
      await agent.aiAssert("界面不存在：最近安装");

    }, { timeout: 600000, tags: ["1832441", "level1", "smoke"] });
