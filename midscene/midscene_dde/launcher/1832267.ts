/**
 * 用例 PMSID: 1832267
 * 用例标题: 【启动器】【窗口模式】【“按名称”分类】【字母表鼠标操作】点击字母表中的字母快速定位至应用列表
 * 生成时间: 2026/1/22 19:41
 * 用例编写人: UT002998(熊林辉)
 */

describe('1832267-【启动器】【窗口模式】【“按名称”分类】【字母表鼠标操作】点击字母表中的字母快速定位至应用列表', () => {
  ...
  //预置步骤5：收尾按ESC关闭启动器，保持环境干净
    ...
    test('1832267-【启动器】【窗口模式】【“按名称”分类】【字母表鼠标操作】点击字母表中的字母快速定位至应用列表', async ({ device, agent, uos }) => {
      //步骤1：打开启动器
      await uos.openLauncher();
    ...
    // 步骤 5: 点击字母C类
      await agent.aiTap('C');
       //D分类被tips遮挡了
      await agent.aiAssert("启动器界面存在分类：C、F、G");

    }, { timeout: 300000, tags: ["1832267", "level2", "smoke"] });
