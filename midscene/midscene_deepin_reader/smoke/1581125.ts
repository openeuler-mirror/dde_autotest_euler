...
describe('1581125-[017]添加高亮的方式', () => {
  ...
  test('1581125-[017]添加高亮的方式', async ({ device, agent, uos }) => {
    // 打开文档查看器应用程序
    ...
    // 拖拽选中一段未高亮的文字内容
    console.log('拖拽选中一段未高亮的文字内容');
    await agent.aiDrag('测试', '文字', { deepThink: true });
    await new Promise(resolve => setTimeout(resolve, 2000));

    // 右键点击选中的文字区域，验证取消高亮选项为置灰状态
    console.log('右键点击选中文字，验证取消高亮选项为置灰状态');
    await agent.aiRightClick('选中的文字');
    await new Promise(resolve => setTimeout(resolve, 1000));
    await agent.aiAssert('取消高亮菜单选项显示为置灰状态，无法被选中');

    // 在右键菜单中点击高亮选项
    console.log('在右键菜单中点击高亮选项');
    await agent.aiWaitFor('高亮');
    await agent.aiTap('高亮');
    await new Promise(resolve => setTimeout(resolve, 2000));

    // 验证选中的文字背景变为高亮颜色
    await agent.aiAssert('选中的文字背景变为高亮颜色');

  }, { timeout: 1200000, tags: ['1581125', 'level1', 'smoke', 'DITT', 'yangtong'] });
