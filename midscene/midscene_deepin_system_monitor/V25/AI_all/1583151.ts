...
/**
 * 用例 PMSID: 1583151
 * 用例标题:舒展模式单击左侧处理器跳转详情页
 * 生成时间: 2025-11-19 09:50:26
 * 用例编写人：UT004526(赵培蕾)
 */

describe('1583151-舒展模式单击左侧处理器跳转详情页', () => {
    ...
    test('1583151-舒展模式单击左侧处理器跳转详情页', async ({ device, agent, uos}) => {
      //步骤  1：打开系统监视器,显示程序进程页界面
      await uos.openApp("系统监视器", 2000, 40000,false);
      await agent.aiAssert("窗口右侧显示程序进程列表");
      //步骤 2: 单击左侧处理器模块
      await agent.aiTap("窗口左侧'处理器'");
      //步骤 3: 程序进程列表切换到处理器详情界面
      await agent.aiAssert("窗口右侧显示处理器详情界面");
      await agent.aiTap("窗口右上角关闭按钮:X");
    }, { timeout: 600000, tags: ['1583151','level2','smoke'] });  
