...
/**
 * 用例 PMSID: 1583155
 * 用例标题:紧凑模式单击左侧磁盘跳转详情页
 * 生成时间: 2025-11-19 09:50:26
 * 用例编写人：UT004526(赵培蕾)
 */

describe('1583155-紧凑模式单击左侧磁盘跳转详情页', () => {
    ...
    test('1583155-紧凑模式单击左侧磁盘跳转详情页', async ({ device, agent, uos}) => {
      //步骤  1：打开系统监视器,显示程序进程页界面
      await uos.openApp("系统监视器", 2000, 40000,false);
      await agent.aiAssert("窗口右侧显示程序进程列表");
      //步骤 2: 单击左侧磁盘模块
      await agent.aiTap("窗口左侧'磁盘'");
      //步骤 3: 程序进程列表切换到磁盘详情界面
      await agent.aiAssert("窗口右侧上半部分显示磁盘IO速率图,下半部分显示磁盘详情");
    }, { timeout: 600000, tags: ['1583155','level2','smoke'] });  
