...
describe('1810933-埋点，侧边栏各目录去重后的点击分布-系统盘', () => {
  ...
  test('1810933-埋点，侧边栏各目录去重后的点击分布-系统盘', async ({ device, agent, uos, system }) => {
    // 获取埋点日志文件路径
    ...
    // 步骤1：打开文管，点击左侧边栏-系统盘
    console.log('步骤1：打开文件管理器，点击侧边栏系统盘');
    await uos.openApp('文件管理器', { maximizeWindow: true });
    await agent.aiTap('左侧边栏中的系统盘', { deepThink: true });

    // 断言1：切换到系统盘目录
    console.log('断言1：验证已切换到系统盘目录');
    await agent.aiAssert('当前已切换到系统盘目录', { deepThink: true });
    console.log('断言1通过：已成功切换到系统盘目录');

    // 等待埋点日志写入
    await new Promise(resolve => setTimeout(resolve, 2000));

    // 步骤2：读取操作后新增的埋点日志
    console.log('步骤2：读取点击系统盘后新增的埋点日志');
    const newLogs = await readNewLogs(system, logFile, logSizeBefore);
    console.log(`点击系统盘后新增的埋点日志:\n${newLogs}`);

    if (!newLogs) {
      throw new Error('点击系统盘后未产生新的埋点日志');
    }

    // 断言2：埋点日志包含文管版本、操作日期时间、侧边栏目录名称（System Disk）
    console.log('断言2：验证新增埋点日志上报内容');
    const hasAppVersion = newLogs.includes('app_version');
    const hasSysTime = newLogs.includes('sysTime');
    const hasSidebarItem = newLogs.includes('sidebar_item') && newLogs.includes('System Disk');
    if (!hasAppVersion || !hasSysTime || !hasSidebarItem) {
      console.log(`断言2详情: app_version=${hasAppVersion}, sysTime=${hasSysTime}, sidebar_item=System Disk=${hasSidebarItem}`);
      throw new Error(`断言2失败：新增埋点日志缺少必要字段 - app_version:${hasAppVersion}, sysTime:${hasSysTime}, sidebar_item(System Disk):${hasSidebarItem}`);
    }
    console.log('断言2通过：成功验证新增埋点日志包含文管版本、操作日期时间和侧边栏目录名称System Disk');

  }, { timeout: 600000, tags: ['1810933', 'level3', 'tracking', 'hujian'] });
