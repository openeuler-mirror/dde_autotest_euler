...
describe('1815837-埋点，右键菜单项点击分布-显示方式，列表视图', () => {
  ...
  test('1815837-埋点，右键菜单项点击分布-显示方式，列表视图', async ({ device, agent, uos, system }) => {
    // 步骤1：打开文件管理器，进入桌面目录
    console.log('步骤1：打开文件管理器，进入桌面目录');
    ...
    // 步骤2：右键菜单选择显示方式-列表视图
    console.log('步骤2：右键菜单选择显示方式-列表视图');
    await agent.aiRightClick('文件管理器当前目录的空白区域');
    await agent.aiTap('右键菜单中的显示方式');
    await agent.aiTap('二级菜单中的列表视图');

    // 断言1：当前目录显示为列表视图
    console.log('断言1：验证当前目录显示为列表视图');
    await agent.aiAssert('当前目录以列表视图方式显示', { deepThink: true });
    console.log('断言1通过：当前目录已切换为列表视图');

    // 步骤3：读取操作后新增的埋点日志
    console.log('步骤3：读取操作后新增的埋点日志');
    const newLogs = await readNewLogs(system, logFile, logSizeBefore);
    console.log(`操作后新增的埋点日志:\n${newLogs}`);

    if (!newLogs) {
      throw new Error('操作后未产生新的埋点日志');
    }

    // 断言2：新增埋点日志包含文管版本、操作日期时间、选项名称(item_name=列表视图)、选中文件类型(type=inode/directory)、进程名称(dde-file-manager/dde-desktop)、触发位置(location)
    console.log('断言2：验证新增埋点日志上报内容');
    const hasAppVersion = newLogs.includes('app_version');
    const hasSysTime = newLogs.includes('sysTime');
    const hasItemName = newLogs.includes('item_name') && newLogs.includes('列表视图');
    const hasType = newLogs.includes('type') && newLogs.includes('inode/directory');
    ...
    if (!hasAppVersion || !hasSysTime || !hasItemName || !hasType || !hasProcess || !hasLocation) {
      ...
      throw new Error(`断言2失败：新增埋点日志缺少必要字段 - app_version:${hasAppVersion}, sysTime:${hasSysTime}, item_name:${hasItemName}, type:${hasType}, process:${hasProcess}, location:${hasLocation}`);
    }
    console.log('断言2通过：成功验证新增埋点日志包含文管版本、操作日期时间、选项名称(列表视图)、文件类型(inode/directory)、进程名称、触发位置');

  }, { timeout: 600000, tags: ['1815837', 'level3', 'tracking', 'hujian'] });
