/**
 * 用例 PMSID: 2003941
 * 用例标题: 埋点，侧边栏各目录去重后的点击分布-挂载smb共享文件夹
...
describe('2003941-埋点，侧边栏各目录去重后的点击分布-挂载smb共享文件夹', () => {
  ...
  test('2003941-埋点，侧边栏各目录去重后的点击分布-挂载smb共享文件夹', async ({ device, agent, uos, system }) => {
    ...
    const smbUsername = process.env.SMB_USERNAME;
    const smbPassword = process.env.SMB_PASSWORD;

    if (!smbIp || !smbDir || !smbUsername || !smbPassword) {
      throw new Error('缺少SMB环境变量配置：SMB_IP, SMB_DIR, SMB_USERNAME, SMB_PASSWORD');
    }

    // 步骤1：前置清理smb挂载
    console.log('步骤1：前置清理smb挂载');
    ...
    // 步骤2：挂载smb
    console.log('步骤2：挂载smb');
    const { SmbMount } = await import(`${caseDir}/midscene_dde_file_manager/common/common.ts`);
    await SmbMount(agent, system, device, 1);

    // 断言1：挂载成功
    console.log('断言1：验证smb挂载成功');
    await agent.aiAssert(`侧边栏存在${smbIp}挂载项`, { deepThink: true });
    console.log('断言1通过：smb挂载成功');
    ...
    // 步骤3：点击侧边栏smb地址项
    console.log('步骤3：点击侧边栏smb地址项');
    await agent.aiTap('侧边栏中的计算机');
    await agent.aiTap(`侧边栏中的${smbIp}文本`, { deepThink: true });
    await agent.aiWaitFor('已进入smb共享目录');

    // 等待埋点日志写入
    await new Promise(resolve => setTimeout(resolve, 2000));

    // 步骤4：读取操作后新增的埋点日志
    console.log('步骤4：读取点击smb后新增的埋点日志');
    const newLogs = await readNewLogs(system, logFile, logSizeBefore);
    console.log(`点击smb后新增的埋点日志:\n${newLogs}`);

    if (!newLogs) {
      throw new Error('点击smb后未产生新的埋点日志');
    }

    // 断言2：埋点日志包含文管版本、操作日期时间、侧边栏目录名称（smb地址）
    console.log('断言2：验证新增埋点日志上报内容');
    const hasAppVersion = newLogs.includes('app_version');
    const hasSysTime = newLogs.includes('sysTime');
    const hasSidebarItem = newLogs.includes('sidebar_item') && newLogs.includes(smbIp);
    if (!hasAppVersion || !hasSysTime || !hasSidebarItem) {
      console.log(`断言2详情: app_version=${hasAppVersion}, sysTime=${hasSysTime}, sidebar_item=${smbIp}=${hasSidebarItem}`);
      throw new Error(`断言2失败：新增埋点日志缺少必要字段 - app_version:${hasAppVersion}, sysTime:${hasSysTime}, sidebar_item(${smbIp}):${hasSidebarItem}`);
    }
    console.log('断言2通过：成功验证新增埋点日志包含文管版本、操作日期时间和侧边栏目录名称');

  }, { timeout: 600000, tags: ['2003941', 'level3', 'tracking', 'hujian'] });
  ...
  afterAll(async ({ uos, agent, device, system }) => {
    ...
    try {
      if (smbIp) {
        ...
        console.log('已卸载smb挂载');
      }
    } catch (err) {
      console.error('卸载smb失败:', err);
    }
