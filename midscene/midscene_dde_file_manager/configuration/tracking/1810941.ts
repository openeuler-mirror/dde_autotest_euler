...
describe('1810941-埋点，侧边栏各目录去重后的点击分布-下载', () => {
  ...
  test('1810941-埋点，侧边栏各目录去重后的点击分布-下载', async ({ device, agent, uos, system }) => {
    // 获取埋点日志文件路径
    ...
    // 步骤1：打开文管，点击左侧边栏-下载
    console.log('步骤1：打开文件管理器，点击侧边栏下载');
    await uos.openApp('文件管理器', { maximizeWindow: true });
    await agent.aiTap('左侧边栏中的下载', { deepThink: true });

    // 断言1：切换到下载目录
    console.log('断言1：验证已切换到下载目录');
    await agent.aiAssert('当前已切换到下载目录', { deepThink: true });
    console.log('断言1通过：已成功切换到下载目录');

    // 等待埋点日志写入
    await new Promise(resolve => setTimeout(resolve, 2000));

    // 步骤2：读取操作后新增的埋点日志
    console.log('步骤2：读取点击下载后新增的埋点日志');
    const newLogs = await readNewLogs(system, logFile, logSizeBefore);
    console.log(`点击下载后新增的埋点日志:\n${newLogs}`);

    if (!newLogs) {
      throw new Error('点击下载后未产生新的埋点日志');
    }

    // 断言2：埋点日志包含文管版本、操作日期时间、侧边栏目录名称（Downloads）
    console.log('断言2：验证新增埋点日志上报内容');
    const hasAppVersion = newLogs.includes('app_version');
    const hasSysTime = newLogs.includes('sysTime');
    const hasSidebarItem = newLogs.includes('sidebar_item') && (newLogs.includes('Downloads') || newLogs.includes('下载'));
    if (!hasAppVersion || !hasSysTime || !hasSidebarItem) {
      console.log(`断言2详情: app_version=${hasAppVersion}, sysTime=${hasSysTime}, sidebar_item=Downloads/下载=${hasSidebarItem}`);
      throw new Error(`断言2失败：新增埋点日志缺少必要字段 - app_version:${hasAppVersion}, sysTime:${hasSysTime}, sidebar_item(Downloads/下载):${hasSidebarItem}`);
    }
    console.log('断言2通过：成功验证新增埋点日志包含文管版本、操作日期时间和侧边栏目录名称Downloads/下载');

  }, { timeout: 600000, tags: ['1810941', 'level3', 'tracking', 'hujian'] });
