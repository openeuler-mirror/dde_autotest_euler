/**
 * 用例 PMSID: 2003945
 * 用例标题: 埋点，侧边栏各目录去重后的点击分布-挂载sftp共享文件夹
...
describe('2003945-埋点，侧边栏各目录去重后的点击分布-挂载sftp共享文件夹', () => {
  ...
  test('2003945-埋点，侧边栏各目录去重后的点击分布-挂载sftp共享文件夹', async ({ device, agent, uos, system }) => {
    ...
    if (!sftpIp || !sftpUsername || !sftpPassword) {
      throw new Error('缺少SFTP环境变量配置：SFTP_IP, SFTP_USERNAME, SFTP_PASSWORD');
    }

    // 步骤1：前置清理sftp挂载
    console.log('步骤1：前置清理sftp挂载');
    ...
    // 步骤2：挂载sftp
    console.log('步骤2：挂载sftp');
    const { SftpMount } = await import(`${caseDir}/midscene_dde_file_manager/common/common.ts`);
    await SftpMount(agent, system, device);

    // 断言1：挂载成功
    console.log('断言1：验证sftp挂载成功');
    await agent.aiAssert(`侧边栏存在${sftpIp}挂载项`, { deepThink: true });
    console.log('断言1通过：sftp挂载成功');
    ...
    // 步骤3：点击侧边栏sftp地址项
    console.log('步骤3：点击侧边栏sftp地址项');
    await agent.aiTap('侧边栏中的计算机');
    await agent.aiTap(`侧边栏中的${sftpIp}文本`, { deepThink: true });
    ...
    // 步骤4：读取操作后新增的埋点日志
    console.log('步骤4：读取点击sftp后新增的埋点日志');
    const newLogs = await readNewLogs(system, logFile, logSizeBefore);
    console.log(`点击sftp后新增的埋点日志:\n${newLogs}`);

    if (!newLogs) {
      throw new Error('点击sftp后未产生新的埋点日志');
    }

    // 断言2：埋点日志包含文管版本、操作日期时间、侧边栏目录名称（sftp地址）
  ...
  }, { timeout: 600000, tags: ['2003945', 'level3', 'tracking', 'hujian'] });
  ...
  afterAll(async ({ uos, agent, device, system }) => {
    ...
    try {
      if (sftpIp) {
        const { cleanSftpMounts } = await import(`${caseDir}/midscene_dde_file_manager/common/common.ts`);
        await cleanSftpMounts(agent, system);
        console.log('已卸载sftp挂载');
      }
    } catch (err) {
      console.error('卸载sftp失败:', err);
    }
