/**
 * 用例 PMSID: 1811441
 * 用例标题: 暴力输入-输入错误密码过程中输入正确密码
 * 生成时间: 2026-2-6 17:00:00
 * 用例编写人: UT005160(蓝雁玲)
 */

const caseDir = process.env.TESTCASE_DIR;

describe('1811441-暴力输入-输入错误密码过程中输入正确密码', () => {
  ...
  test('1811441-暴力输入-输入错误密码过程中输入正确密码', async ({ device, agent, uos, env, system }) => {    
    ...
    //步骤2~4：第一次到三次输入错误密码解锁保险箱   
    for (let i = 6; i >= 4; i--) {
      await agent.aiTap("有解锁文字的按钮");

      await agent.aiWaitFor(`有错误提示:密码错误，您还可再输入${i-1}次或者输入框变为粉色或者输入框变为褐色或者输入框变为红色`,
        {
          timeoutMs: 60000,
    ...
    //步骤5：输入正确密码解锁保险箱
    await agent.aiTap("当前保险箱窗口输入框中的x按钮");
    ...
    await agent.aiInput("ABc123!!","含有密码文字的输入框");
    await agent.aiTap("有解锁文字的按钮");
    await agent.aiWaitFor("有错误提示:密码错误，您还可再输入5次或者输入框变为粉色或者输入框变为褐色或者输入框变为红色",
      {
        timeoutMs: 60000,
        checkIntervalMs: 5000
      }
  ...
  }, { timeout: 1200000, tags: ['1811441','level3','main_interface_area','fixed_directory','vault','DITT','lanyanling'] });
