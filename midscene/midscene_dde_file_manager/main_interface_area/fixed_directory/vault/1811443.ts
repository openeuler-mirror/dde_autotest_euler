 ...
 * 用例 PMSID: 1811443
 * 用例标题: 暴力输入-保险箱锁定时间等待
 * 生成时间: 2026-2-5 21:00:00
 * 用例编写人: UT005160(蓝雁玲)
 */

const caseDir = process.env.TESTCASE_DIR;

describe('1811443-暴力输入-保险箱锁定时间等待', () => {
  ...
  test('1811443-暴力输入-保险箱锁定时间等待', async ({ device, agent, uos, env, system }) => {    
    ...
    //步骤1：双击计算机页面保险箱/文件选择对话框保险箱-连续输入错误密码6次
    ...
    //步骤2：在当前页面等待7分钟-再次输入错误密码
    await new Promise(resolve => setTimeout(resolve, 420000));
    await agent.aiTap("有解锁文字的按钮");
    await agent.aiWaitFor("有错误提示:请3分钟后再试或者输入框变为粉色",
      {
        timeoutMs: 60000,
        checkIntervalMs: 5000
      }
    ...
    //步骤3：在当前页面继续等待到10分钟-再次输入错误密码  
    await new Promise(resolve => setTimeout(resolve, 180000));
    await agent.aiTap("有解锁文字的按钮");
    //提示密码错误，请100分钟后再试
    await agent.aiWaitFor("有错误提示:请100分钟后再试或者输入框变为粉色",
      {
        timeoutMs: 60000,
        checkIntervalMs: 5000
      }
  ...
  }, { timeout: 1700000, tags: ['1811443','level3','main_interface_area','fixed_directory','vault','DITT','lanyanling'] });
