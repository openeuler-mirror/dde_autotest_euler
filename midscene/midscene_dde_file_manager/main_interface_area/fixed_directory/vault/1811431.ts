/**
 * 用例 PMSID: 1811431
 * 用例标题:  暴力输入-输入错误密码过程中手动杀掉文管进程
 * 生成时间: 2026-2-9 10:00:00
 * 用例编写人: UT005160(蓝雁玲)
 */

const caseDir = process.env.TESTCASE_DIR;

describe('1811431- 暴力输入-输入错误密码过程中手动杀掉文管进程', () => {
  ...
  test('1811431- 暴力输入-输入错误密码过程中手动杀掉文管进程', async ({ device, agent, uos, env, system }) => {    
    ...
    //步骤5：在终端sudo killall dde-file-manager杀掉文管进程
    await system.exec("ps aux |grep dde-file-manager | grep -v grep | awk '{print $2}' | xargs kill -15");

    //步骤6~8：在新窗口后第一次~第三次输入错误密码解锁保险箱
    await uos.openApp("文件管理器", 3000, 20000, true);
    await agent.aiTap("文件管理器左侧栏的保险箱");
    await agent.aiTap("含有密码文字的输入框");
    await agent.aiInput("ABc123!!","含有密码文字的输入框");   
  ...
  }, { timeout: 1200000, tags: ['1811431','level3','main_interface_area','fixed_directory','vault','DITT','lanyanling'] });
