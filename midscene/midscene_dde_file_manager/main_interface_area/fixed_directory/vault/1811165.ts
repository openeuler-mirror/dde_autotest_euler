/**
 * 用例 PMSID: 1811165
 * 用例标题: 指定路径密码找回-通过指定路径密钥文件找回密码默认选项
 * 生成时间: 2026-05-26 19:30:00
 * 用例编写人: UT000159（游伟）
 */

const caseDir = process.env.TESTCASE_DIR;

describe('1811165-指定路径密码找回-通过指定路径密钥文件找回密码默认选项', () => {
  ...
  test('1811165-指定路径密码找回-通过指定路径密钥文件找回密码默认选项', async ({ device, agent, uos, env, system }) => {
    ...
    // 预期 5: 当前界面是使用密钥解锁保险箱页面
    console.log('预期 5: 当前界面是使用密钥解锁保险箱页面');
    await agent.aiAssert('当前界面是使用密钥解锁保险箱页面');

    // 步骤 6: 点击密钥解锁按钮
    console.log('步骤 6: 点击密钥解锁按钮');
    await agent.aiTap("使用密钥解锁按钮");

    // 预期 6: 当前界面是使用密钥解锁保险箱页面
    console.log('预期 6: 当前界面是使用密钥解锁保险箱页面');
    await agent.aiAssert('当前界面是使用密钥解锁保险箱页面');

    // 步骤 7: 点击返回按钮
    console.log('步骤 7: 点击返回按钮');
    await agent.aiTap('使用密钥解锁保险箱页面中的返回按钮');

    // 预期 7: 当前界面是解锁保险箱页面
    console.log('预期 7: 当前界面是解锁保险箱页面');
    await agent.aiAssert('当前界面是解锁保险箱页面');

  }, { timeout: 1200000, tags: ['1811165', 'level3', 'main interface area', 'fixed directory', 'vault', 'DITT', 'youwei', 'file-manager', 'restore password', 'default selection'] });
});