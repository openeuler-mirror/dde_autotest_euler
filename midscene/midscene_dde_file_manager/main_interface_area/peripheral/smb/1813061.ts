...
/**
 * 用例 PMSID: 1813061
 * 用例标题: 文管设置：显示 Samba共享常驻，所有目录都为卸载状态
 * 生成时间: 2026-02-27 17:08:20
 * 用例编写人：UT006252(杨通)
 */

describe('1813061-文管设置：显示 Samba共享常驻，所有目录都为卸载状态', () => {
  beforeAll(async ({ device, uos, agent, system }) => {
    ...
    //修改组策略值为false
    await system.exec(`dde-dconfig --set -a org.deepin.dde.file-manager -r org.deepin.dde.file-manager -k dfm.samba.permanent -v false`);
    //卸载smb
  ...
  test('1813061-文管设置：显示 Samba共享常驻，所有目录都为卸载状态', async ({ device, agent, uos ,system}) => {
    const ip=process.env.SMB_IP;
    //返回计算机界面
    ...
    console.log('✅ 验证通过：配置项值为 true');} 
    else {
    // 失败时打印实际值，方便排查
    console.error(`❌ 验证失败：期望 'true'，实际得到 '${result.stdout}'`);
    throw new Error(`配置项检查失败：预期为 true，但实际值为 ${result.stdout}`);}
  }, { timeout: 1200000, tags: ['1813061', 'level3','smb','DITT','yangtong'] });
