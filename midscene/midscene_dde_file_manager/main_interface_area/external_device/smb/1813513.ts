...
/**
 * 用例 PMSID: 1813513
 * 用例标题: Bug211337转：拷贝SMB内的多层次目录到本地
...
describe('1813513-Bug211337转：拷贝SMB内的多层次目录到本地', () => {
  ...
  test('1813513-Bug211337转：拷贝SMB内的多层次目录到本地', async ({ device, agent, uos, system, env }) => {
    ...
    await system.exec(`mkdir -p /media/$USER/smbmounts/smb-share:server=${process.env.SMB_IP},share=smbtest/hu123/qw/er/ty/ui/op/as/df/gh/jk/zx`, 500);
    await agent.aiWaitFor("文管窗口内存在hu123 文件夹");
    for(i=0;i<6;i++){
      await system.exec(`mkdir /media/$USER/smbmounts/smb-share:server=${process.env.SMB_IP},share=smbtest/hu123/qw/er/ty/ui/op/as/df/gh/jk/zx/hu${i}`, 500);
      await system.exec(`touch /media/$USER/smbmounts/smb-share:server=${process.env.SMB_IP},share=smbtest/hu123/qw/er/ty/ui/op/as/df/gh/jk/zx/te${i}.txt`, 500);
    }
    await agent.aiDoubleClick("hu123");
    await agent.aiDoubleClick("qw");
    await agent.aiRightClick("er");
    await agent.aiTap('右键菜单中复制', { deepThink: true });
    await system.cleanupFileManager();
    await uos.showDesktop();

    await agent.aiRightClick("桌面中心");
    await agent.aiTap('右键菜单中粘贴', { deepThink: true });
    await agent.aiAssert("桌面多出一个名为er的文件夹");

  }, { timeout: 1200000, tags: ['1813513', 'level3', 'smb', 'huhongjie'] });
  ...
  afterAll(async ({ uos, agent, device, system ,env, common}) => {
    ...
    await system.exec(`rm -rf ~/Desktop/er`, 500);
