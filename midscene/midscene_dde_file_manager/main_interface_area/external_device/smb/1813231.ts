...
describe('1813231-挂载smb-输入错误密码不能访问smb', () => {
  ...
  test('1813231-挂载smb-输入错误密码不能访问smb', async ({ device, agent, uos, system,env }) => {

    await system.exec(`mkdir ~/Desktop/1813231 `, 500);
    // await system.exec(`touch ~/Desktop/1813231/1813231.txt`, 500);
    const ipResult = await system.exec(`hostname -I | awk '{print $1}'`);
    const ip = ipResult.stdout.trim();
    console.log(`获取到本机IP为：${ip}`);
    await agent.aiRightClick('1813231', 300);
    ...
    await agent.aiAssert("文管窗口内存在文件夹1813231");
    await agent.aiDoubleClick('1813231');
    await agent.aiAssert("显示需要授权来访问弹窗");
    await device.typeText('asd');
    await agent.aiTap('密码输入框', { deepThink: true });
  ...
  }, { timeout: 1200000, tags: ['1813231', 'level2', 'smb', 'huhongjie'] });
