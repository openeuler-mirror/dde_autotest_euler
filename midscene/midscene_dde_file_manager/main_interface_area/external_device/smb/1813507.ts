...
describe('1813507-右键菜单-侧边栏smb目录右键-属性', () => {
  ...
  test('1813507-右键菜单-侧边栏smb目录右键-属性', async ({ device, agent, uos, system, env }) => {
    ...
    await agent.aiTap('左侧边栏计算机', { deepThink: true });
    await agent.aiDoubleClick(`文管窗口内${process.env.SMB_IP}`, { deepThink: true });
    await agent.aiRightClick("SmbTest");
    await agent.aiTap('属性');
    await agent.aiAssert('正常弹出属性弹窗，显示该smb目录的图标、文件名、使用容量/总容量、基本信息：设备类型、总容量、文件系统、文件个数、可用空间');

  }, { timeout: 1200000, tags: ['1813507', 'level2', 'smb', 'huhongjie'] });
