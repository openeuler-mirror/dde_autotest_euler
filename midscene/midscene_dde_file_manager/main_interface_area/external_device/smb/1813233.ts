...
describe('1813233-挂载smb-未设置共享密码时访问挂载', () => {
  ...
  test('1813233-挂载smb-未设置共享密码时访问挂载', async ({ device, agent, uos, system,env }) => {

    await system.exec(`mkdir ~/Desktop/1813233 `, 500);
    await system.exec(`touch ~/Desktop/1813233/1813233.txt`, 500);
    const ipResult = await system.exec(`hostname -I | awk '{print $1}'`);
    const ip = ipResult.stdout.trim();
    console.log(`获取到本机IP为：${ip}`);
    await agent.aiRightClick('1813233', 300);
    ...
    await agent.aiAssert("文管窗口内存在文件夹1813233");
    await agent.aiDoubleClick('1813233');
    await agent.aiAssert("显示需要授权来访问弹窗");
    await agent.aiTap('密码输入框', { deepThink: true });
    await device.typeText('1813233');
    await agent.aiTap('连接');
    await agent.aiAssert("弹出挂载失败提示弹窗");
    await device.pressKey('Esc');
    
  }, { timeout: 1200000, tags: ['1813233', 'level2', 'smb', 'huhongjie'] });
