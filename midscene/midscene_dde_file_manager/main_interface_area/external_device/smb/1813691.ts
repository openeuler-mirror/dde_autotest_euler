...
describe('1813691-挂载smb后，点击顶部的面包屑', () => {
  ...
  test('1813691-挂载smb后，点击顶部的面包屑', async ({ device, agent, uos, system, env }) => {
...
await system.exec(`mkdir -p /media/$USER/smbmounts/smb-share:server=${process.env.SMB_IP},share=smbtest/hu123/ut000374/qwe/1813691/zxc/456/asd `, 500);
   
    await agent.aiWaitFor("hu123");
    await agent.aiDoubleClick("hu123");
    await agent.aiDoubleClick("ut000374");
    await agent.aiDoubleClick("qwe");
    await agent.aiDoubleClick("1813691");
    await agent.aiDoubleClick("zxc");
    await agent.aiDoubleClick("456");
    await agent.aiDoubleClick("asd");
    await agent.aiAssert("进入asd目录中");
    console.log('开始操作顶部路径！'); 
    await agent.aiTap('文管窗口顶部zxc', { deepThink: true });
    await agent.aiAssert("进入zxc目录中");
    await agent.aiTap('文管窗口顶部1813691', { deepThink: true });
    await agent.aiAssert("进入1813691目录中");
    await agent.aiTap('文管窗口顶部ut000374', { deepThink: true });
    await agent.aiAssert("进入ut000374目录中");

  }, { timeout: 1200000, tags: ['1813691', 'level3', 'smb', 'huhongjie'] });
