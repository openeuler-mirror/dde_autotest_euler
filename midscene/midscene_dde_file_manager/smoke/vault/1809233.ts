...
/**
 * 用例 PMSID: 1809233
 * 用例标题: [124]保险箱-已开启侧边栏/计算机目录呼出右键菜单正常
 * 生成时间: 2026-01-15 19:49:15
 * 用例编写人：UT006252(杨通)
 */

describe('1809233-[124]保险箱-已开启侧边栏/计算机目录呼出右键菜单正常', () => {
  ...
  test('1809233-[124]保险箱-已开启侧边栏/计算机目录呼出右键菜单正常', async ({ device, agent, uos,system }) => {
    //返回计算机页面
    await system.exec(`sleep 3`);
    await agent.aiTap('侧边栏的计算机选项')
    await agent.aiWaitFor('我的目录文本');
    await agent.aiRightClick('保险箱图标')
    await agent.aiWaitFor('出现菜单');
    await agent.aiAssert("菜单包含打开、在新窗口打开、立即上锁、自动上锁、重置密码、删除保险箱、属性");
    await agent.aiDoubleClick("页面空白处");
    await agent.aiRightClick('侧边栏的保险箱选项')
    await agent.aiWaitFor('出现菜单');
    await agent.aiAssert("菜单包含打开、在新窗口打开、立即上锁、自动上锁、重置密码、删除保险箱、属性");
  }, { timeout: 1200000, tags: ['1809233', 'level2','smoke','DITT','yangtong'] });
