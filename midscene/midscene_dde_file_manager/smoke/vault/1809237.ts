...
/**
 * 用例 PMSID: 1809237
 * 用例标题: [125]保险箱-侧边栏/计算机目录右键自动上锁默认检查
 * 生成时间: 2026-01-15 19:29:56
 * 用例编写人：UT006252(杨通)
 */

describe('1809237-[125]保险箱-侧边栏/计算机目录右键自动上锁默认检查', () => {
  ...
  test('1809237-[125]保险箱-侧边栏/计算机目录右键自动上锁默认检查', async ({ device, agent, uos }) => {
    //检查自动上锁设置
    await agent.aiRightClick("保险箱文本");
    await agent.aiHover('自动上锁选项');
    await agent.aiWaitFor('出现二级菜单栏');
    await agent.aiAssert("自动上锁的可选项为从不、5分钟、10分钟、20分钟");
    await agent.aiAssert("自动上锁选项勾选从不");
  }, { timeout: 1200000, tags: ['1809237', 'level2','smoke','DITT','yangtong'] });
