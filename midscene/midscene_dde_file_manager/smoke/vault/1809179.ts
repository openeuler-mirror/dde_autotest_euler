...
/**
 * 用例 PMSID: 1809179
 * 用例标题: 【保险箱】保险箱入口右键菜单-检查计算机页面/侧边栏右键菜单
 * 生成时间: 2026-01-15 20:17:30
 * 用例编写人：UT006252(杨通)
 */

describe('1809179-【保险箱】保险箱入口右键菜单-检查计算机页面/侧边栏右键菜单', () => {
  ...
  test('1809179-【保险箱】保险箱入口右键菜单-检查计算机页面/侧边栏右键菜单', async ({ device, agent, uos}) => {
    const TEST_PASSWORD = process.env.TEST_PASSWORD;
    
    //上锁保险箱
    console.log('上锁保险箱');
    await agent.aiRightClick("保险箱选项");
    await agent.aiWaitFor("出现菜单栏");
    await agent.aiTap('立即上锁选项');
    await agent.aiWaitFor('我的目录文本');
    await agent.aiRightClick('保险箱图标');
    await agent.aiWaitFor('出现菜单');
    await agent.aiAssert("菜单包含解锁、重置密码");
    await agent.aiTap('侧边栏的保险箱选项');

    //查看右键菜单并删除保险箱
    await agent.aiRightClick('侧边栏的保险箱选项')
    await agent.aiWaitFor('出现菜单');
    await agent.aiAssert("菜单包含解锁、重置密码");
    await agent.aiTap('解锁选项');
    await agent.aiInput('Aa!12345','密码输入框');
  ...
  }, { timeout: 1200000, tags: ['1809179', 'level2','smoke','DITT','yangtong'] });
