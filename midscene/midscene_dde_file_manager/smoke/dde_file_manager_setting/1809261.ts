...
describe('1809261-盘符名称修改-重命名输入框唤起及保存-重命名', () => {
  ...
  test('1809261-盘符名称修改-重命名输入框唤起及保存-重命名', async ({ device, agent, uos }) => {
    // 步骤 1: 打开文件管理器
    await uos.openApp('文件管理器');
    await agent.aiWaitFor("文件管理器界面已显示");
    // 步骤 2: 右键系统盘，点击重命名
    await agent.aiRightClick("磁盘列表下方的系统盘");
    await agent.aiTap("右键菜单中的重命名");
    // 步骤 3: 显示重命名输入框
    await agent.aiAssert("系统盘名称背景显示蓝色");

  }, { timeout: 600000,
       tags: ['1809261', 'level2', 'smoke', 'midscene_dde_file_manager/smoke/dde_file_manager_setting', 'DITT', 'hujian'] });
