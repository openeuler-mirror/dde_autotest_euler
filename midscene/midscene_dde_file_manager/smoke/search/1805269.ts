 ...
 * 用例 PMSID: 1805269
 * 用例标题: 【搜索】高级搜索-打开/关闭
 * 生成时间: 2025-12-18 20:56:26
 * 用例编写人: UT005160(蓝雁玲)
 */

describe('1805269-【搜索】高级搜索-打开/关闭', () => {
  ...
  test('1805269-【搜索】高级搜索-打开/关闭', async ({ device, agent, uos, env, system }) => {
...
*/  
    ...
    await agent.aiTap("右上角输入框右边的漏斗图标");
    await agent.aiAssert("主目录页面有如下关键字：搜索范围、文件类型、访问时间、重置、文件大小、修改时间、创建时间");
    await agent.aiTap("右上角输入框右边的漏斗图标");
    await agent.aiAssert("主目录页面不存在如下关键字：搜索范围、文件类型、访问时间、重置");
    
    //在高级搜索收起状态下按：Ctrl+f
    await device.pressKey('Ctrl+f');
    await agent.aiAssert("主目录页面有如下关键字：搜索范围、文件类型、访问时间、重置、文件大小、修改时间、创建时间");
    await agent.aiTap("当前页面空白处");
    //在高级搜索展开状态下按：Ctrl+f
    //await device.pressKey('Ctrl+f');
    //await agent.delay(3000);
    //await agent.aiAssert("主目录页面不存在如下关键字：搜索范围、文件类型、访问时间、重置");
  }, { timeout: 1200000, tags: ['1805269','level1','smoke','lanyanling'] });
