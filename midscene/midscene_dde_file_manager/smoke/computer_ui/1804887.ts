/**
 * 用例 PMSID: 1804887
 * 用例标题: 侧边栏显示项目设置取消勾选网络分组下所有设置项目
 * 生成时间: 2025-12-17 09:53:00
 * 用例编写人: UT000211
 */


describe('1804887-侧边栏显示项目设置取消勾选网络分组下所有设置项目', () => {
  ...
  test('1804887-侧边栏显示项目设置取消勾选网络分组下所有设置项目', async ({ uos, agent, device }) => {
    ...
    // 3.向下滑动进度条，找到设置页面的网络模块，取消勾选网络标签下的所有项目
    await agent.aiScroll('设置页面侧边栏显示项目详情区域', { direction: 'down', distance: 4 });
    await agent.aiTap('设置页面网络邻居前方的方框');
    await agent.aiAssert('网络下的项目前方方框没有被勾选');
    
      
    // 4.查看文管侧边栏
    await agent.aiAssert('文管侧边栏不显示网络');
  ...
  // 5.滑动设置页面到最底部，点击恢复默认
    ...
    await agent.aiAssert('文管侧边栏显示网络');
    
  // 6.关闭设置菜单
    await agent.aiTap('设置页面的关闭按钮');
    await agent.aiAssert('设置菜单被关闭');
     }, { timeout: 300000, tags: ["1804887",'level2', 'smoke','chenyi'] });
