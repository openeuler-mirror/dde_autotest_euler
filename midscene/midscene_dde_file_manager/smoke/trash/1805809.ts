...
/**
 * 用例 PMSID: 1805809
 * 用例标题: [074]右键菜单-回收站内选择多个文件/文件夹右键菜单功能正常
 * 生成时间: 2025-12-22 07:44:53
 * 用例编写人: UT000195(苏姗姗)
 */

describe('1805809-[074]右键菜单-回收站内选择多个文件/文件夹右键菜单功能正常', () => {
  ...
  test('1805809-[074]右键菜单-回收站内选择多个文件/文件夹右键菜单功能正常', async ({ device, agent, uos, system }) => {
    // 清理应用进程（增加容错，无进程时不报错）
    ...
    //给垃圾箱创建文件夹和文件
    system.exec(`mkdir ~/.local/share/Trash/files/test`);
    system.exec(`touch ~/.local/share/Trash/files/test.txt`);
    ...
    await agent.aiAssert("弹出右键菜单");
    console.log('测试用例执行完成');

  }, { timeout: 600000, tags: ['1805809', 'level2', 'smoke','sushanshan'] });
