/**
 * 用例 PMSID: 1807797
 * 用例标题: 文管右键菜单显示快捷键-快捷键功能-桌面快捷键删除（开启普通删除提示）
 * 生成时间: 2025-2-9 10:00:00
 * 用例编写人: UT002411(胡戬)
 */

const TEST_DIR = '~/Desktop'
const COPY_DIR = '~/Desktop/1807797cp'
...
describe('1807797-文管右键菜单显示快捷键-快捷键功能-桌面快捷键删除（开启普通删除提示）', () => {
  ...
  test('1807797-文管右键菜单显示快捷键-快捷键功能-桌面快捷键删除（开启普通删除提示）', async ({ device, agent, uos }) => {
    // 步骤 1: 进入测试文件夹
    await uos.showDesktop();
    // 步骤2：在桌面右键单击一个文件，唤起右键菜单，按快捷键：D
    ...
    await agent.aiAssert("桌面没有file01.txt文件");
    // 步骤3：在桌面右键单击一个文件夹，唤起右键菜单，按快捷键：D
    await agent.aiRightClick('桌面的dir01文件夹')
    await agent.aiWaitFor("显示右键菜单");
    await device.pressKey('D');
    await agent.aiWaitFor("弹出是否删除的提示框");
    await agent.aiTap('提示框右侧的删除按钮');
    await agent.aiAssert("桌面没有dir01文件夹");
    // 步骤4：在桌面选中多个文件、文件夹、应用等，唤起右键菜单，按快捷键：D
    ...
    await agent.aiWaitFor("弹出是否删除的提示框");
    await agent.aiTap('提示框右侧的删除按钮');
    await agent.aiAssert("桌面没有file02.txt、dir02、归档管理器");

  }, { timeout: 600000,
       tags: ['1807797', 'level2', 'smoke', 'file_operations', 'DITT', 'hujian'] });
