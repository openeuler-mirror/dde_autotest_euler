/**
 * 用例 PMSID: 1807803
 * 用例标题: 文管右键菜单显示快捷键-快捷键功能-文管内快捷键复制粘贴
 * 生成时间: 2025-2-3 10:00:00
 * 用例编写人: UT002411(胡戬)
 */

const TEST_DIR = '1807803te'
const COPY_DIR = '1807803cp'
...
describe('1807803-文管右键菜单显示快捷键-快捷键功能-文管内快捷键复制粘贴', () => {
  ...
  test('1807803-文管右键菜单显示快捷键-快捷键功能-文管内快捷键复制粘贴', async ({ device, agent, uos, system }) => {
    // 步骤 1: 进入测试文件夹
    await agent.aiDoubleClick('桌面上的1807803te文件夹');
    await agent.aiWaitFor("文件管理器界面已显示");
    // 步骤2：在文管内右键单击一个文件，唤起右键菜单，按快捷键：C
    await agent.aiRightClick('文件管理器中的file01.txt文件');
    await agent.aiWaitFor("显示右键菜单");
    await device.pressKey('C');
    await device.pressKey('Alt','F4');
    // 步骤3：粘贴文件到测试目录
    await agent.aiDoubleClick('桌面上的1807803cp文件夹');
    await agent.aiRightClick('文件管理器当前目录空白处');
    await agent.aiWaitFor("显示右键菜单");
    await device.pressKey('P');
    await agent.aiAssert("当前目录有file01.txt文件");
    await device.pressKey('Alt','F4');
    // 步骤4：在文管内右键单击一个文件夹，唤起右键菜单，按快捷键：C
    await agent.aiDoubleClick('桌面上的1807803te文件夹');
    ...
    await device.pressKey('Alt','F4');
    // 步骤6：在文管内选中多个文件、文件夹、应用等，唤起右键菜单，按快捷键：C
    await agent.aiDoubleClick('桌面上的1807803te文件夹');
    ...
    await system.exec(`rm -rf ~/Desktop/${COPY_DIR}/*`)
    await agent.aiDoubleClick('桌面上的1807803cp文件夹');
    await agent.aiRightClick('文件管理器当前目录空白处');
    await agent.aiWaitFor("显示右键菜单");
    await device.pressKey('P');
    await agent.aiAssert("当前目录有以下文件：dir01、dir02、file01.txt、file02.txt、服务与支持");
    await device.pressKey('Alt','F4');

  }, { timeout: 600000,
       tags: ['1807803', 'level2', 'smoke', 'file_operations', 'DITT', 'hujian'] });
