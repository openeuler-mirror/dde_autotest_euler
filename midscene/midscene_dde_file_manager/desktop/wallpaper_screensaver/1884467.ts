/**
 * 用例 PMSID: 1884467
 * 用例标题: 【控制中心】【个性化】【主题】bloom主题下更换外观深浅色，会同步更新深浅色壁纸
 * 生成时间: 2026-01-14 10:46:26
 * 用例编写人：UT000054（叶飞）
 */

describe('1884467-【控制中心】【个性化】【主题】bloom主题下更换外观深浅色，会同步更新深浅色壁纸 ', () => {
  ...
  test('1884467-【控制中心】【个性化】【主题】bloom主题下更换外观深浅色，会同步更新深浅色壁纸', async ({ device, agent, uos, system, env }) => {

    //步骤1：打开控制中心,修改主题为bloom
    //设置主题：bloom
    await agent.aiTap("bloom");
    //等到3秒，壁纸设置生效
    await new Promise(resolve => setTimeout(resolve, 3000));
    //检查控制中心个性化主题
    await agent.aiAssert("bloom主题被选中：外边框有方框");
    //检查桌面壁纸
    await system.exec("/usr/lib/deepin-daemon/desktop-toggle");//显示桌面
    await new Promise(resolve => setTimeout(resolve, 2000)); // 等待2秒
    await agent.aiAssert("桌面壁纸是一张蓝色、白色和淡紫色（洋红色）的渐变融合的壁纸");
    //检查锁屏壁纸
    ...
    await system.exec("/usr/lib/deepin-daemon/desktop-toggle");//显示桌面
    await new Promise(resolve => setTimeout(resolve, 2000)); // 等待2秒
    await agent.aiAssert("桌面壁纸是一张绿色极光的壁纸");
    //检查锁屏壁纸
    await device.pressKey("Super+L");
    await agent.aiWaitFor("屏幕被锁定，出现账户和密码输入框");
    await agent.aiAssert("锁屏壁纸背景是一张绿色极光的的图片");
    await device.typeText(env.testPassword);
    await device.pressKey("Enter");
  ...
  }, { timeout: 1800000, tags: ["1884467", "level3", "wallpaper_screensaver", "yefei"] });
