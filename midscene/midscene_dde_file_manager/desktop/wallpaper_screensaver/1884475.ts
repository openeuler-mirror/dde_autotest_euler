/**
 * 用例 PMSID: 1884475
 * 用例标题: 【控制中心】【个性化】【主题】flow主题下更换外观深浅色，会同步更新深浅色壁纸
 * 生成时间: 2026-01-14 16:13:26
 * 用例编写人：UT000054（叶飞）
 */

describe('1884475-【控制中心】【个性化】【主题】flow主题下更换外观深浅色，会同步更新深浅色壁纸', () => {
  beforeAll(async ({ device, uos, system, agent }) => {
    console.log('1. beforeAll: 初始化测试套件');
    // 初始化文管配置和进程
    await system.cleanupFileManager();
    await uos.showDesktop();//顯示桌面
  ...
  test('1884475-【控制中心】【个性化】【主题】flow主题下更换外观深浅色，会同步更新深浅色壁纸', async ({ device, agent, uos, system, env }) => {

    //步骤1：打开控制中心,修改主题为flow
    await agent.aiTap("flow");
    //等到3秒，壁纸设置生效
    await new Promise(resolve => setTimeout(resolve, 3000));
    //检查控制中心个性化主题
    await agent.aiAssert("flow题被选中：外边框有方框");
    //检查桌面壁纸
    await uos.showDesktop();
    await agent.aiAssert("桌面壁纸是一张‌由多个流畅的白色曲线和半透明圆形构成的壁纸");
    //检查锁屏壁纸
    ...
    //检查桌面壁纸
    await system.exec("/usr/lib/deepin-daemon/desktop-toggle");//显示桌面
    await agent.aiAssert("桌面壁纸是一张以‌蓝色‌为主色调的渐变背景图，整体风格简约现代，色彩过渡自然，视觉效果平滑柔和的壁纸");
    //检查锁屏壁纸
    await device.pressKey("Super+L");
    await agent.aiWaitFor("屏幕被锁定，出现账户和密码输入框");
    await agent.aiAssert("锁屏壁纸背景是一张蓝色的图片");
    ...
    await agent.aiAssert("桌面壁纸是一张由浅紫色与白色渐变效果，配以流畅的白色曲线的壁纸");
    //检查锁屏壁纸
    await device.pressKey("Super+L");
    await agent.aiWaitFor("屏幕被锁定，出现账户和密码输入框");
    await agent.aiAssert("锁屏壁纸背景是一张蓝色的图片");
  ...
  }, { timeout: 600000, tags: ["1884475", "level3", "wallpaper_screensaver", "yefei"] });
