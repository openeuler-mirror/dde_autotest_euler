/**
 * 用例 PMSID: 1884485
 * 用例标题:【控制中心】【个性化】【主题】深色外观，设置不同的系统主题，相关主题壁纸都默认在系统壁纸中
 * 生成时间: 2026-01-15 15:08:26 
 * 用例编写人：UT000054（叶飞）
 */

describe('1884485-【控制中心】【个性化】【主题】深色外观，设置不同的系统主题，相关主题壁纸都默认在系统壁纸中', () => {
  ...
  test('1884485-【控制中心】【个性化】【主题】深色外观，设置不同的系统主题，相关主题壁纸都默认在系统壁纸中', async ({ device, agent, uos, system, env }) => {
    ...
    const themeList = [
      {
        ...
        wallpaperDesc: "桌面壁纸是一张绿色极光的壁纸"
      },
      {
        name: "flow",
        //borderColor: "蓝色", // flow主题选中边框为蓝色
        wallpaperDesc: "桌面壁纸是一张以‌蓝色‌为主色调的渐变背景图，整体风格简约现代，色彩过渡自然，视觉效果平滑柔和的壁纸"
      },
      {
        name: "hazy color",
        // borderColor: "玫红色", // hazy color主题选中边框为玫红色
        wallpaperDesc: "桌面壁纸是一张多个大小不一的圆形或环形元素，叠加形成半透明叠层效果的壁纸"
      },
      {
        name: "nirvana",
        //borderColor: "蓝色", // nirvana主题选中边框为蓝色
        wallpaperDesc: "桌面壁纸是一张‌深蓝色带有波浪条纹和UOS字样的壁纸"
      },
      {
        name: "organic glass",
        // borderColor: "蓝色", // organic galss主题选中边框为蓝色
        wallpaperDesc: "桌面壁纸是一张深色背景，带有粉红色、蓝色和紫色等流动的半透明光带的壁纸"
      },
      {
        name: "square",
        // borderColor: "紫色", // square主题选中边框为紫色
        wallpaperDesc: "桌面壁纸是一张以紫色到蓝色的渐变色,带有星球和音乐符号‌的壁纸"
      },
      {
        name: "vintage",
        //  borderColor: "蓝色", // vintage主题选中边框为蓝色
        wallpaperDesc: "桌面壁纸是一张深蓝色星空与沙漠纹理的壁纸"
      }
    ];
    ...
    const openPersonalization = async () => {
      ...
      await agent.aiWaitFor("个性化可见", { timeout: 5000 }); // 增加超时配置
      await agent.aiTap("个性化");
      await agent.aiWaitFor("主题选项可见", { timeout: 5000 });
    };
    ...
    const verifyTheme = async (themeItem) => {
      ...
      await agent.aiAssert(`系统壁纸有一张被选中状态:壁纸外边框有方框`);
    ...
    // 主流程：初始化+遍历所有主题验证
    const initResult = await openPersonalization().catch(error => ({ error }));
    if (initResult && initResult.error) {
      console.error(`初始化过程中出错：${initResult.error.message}`);
    ...
    for (const theme of themeList) {
      ...
      if (themeResult && themeResult.error) {
        console.error(`主题${theme}验证过程中出错：${themeResult.error.message}`);
        throw themeResult.error;
      }
    }

    console.log("所有主题验证完成，测试通过");
  }, { timeout: 1800000, tags: ["1884485", "level3", "wallpaper_screensaver", "yefei"] });
