...
describe('1816785-桌面右键菜单关闭桌面整理 ', () => {
    ...
    beforeEach(async ({ device, agent, uos, system }) => {
      ...
      // 2.桌面新增多个文件
        await system.exec('touch ~/Desktop/1816785.txt', 500);
        await system.exec('touch ~/Desktop/1816785.xlsx', 500);
        await system.exec('touch ~/Desktop/1816785.doc', 500);

    });
  
    test('1816785-桌面右键菜单关闭桌面整理 ', async ({ device, agent, uos, system, env }) => {
        ...
        await agent.aiHover('1816785.txt');
        await agent.aiAssert('文档');

        // 桌面右键菜单-桌面设置-关闭桌面整理
        await agent.aiRightClick('桌面居中位置空白区');
        await agent.aiWaitFor('右键菜单');
        await agent.aiTap('桌面设置');
        await agent.aiWaitFor('启用桌面整理');
        await agent.aiTap('启用桌面整理右侧的蓝色按钮', 1000);
        await agent.aiWaitFor('桌面集合方式被隐藏');
    ...
    }, { timeout: 1200000, tags: ["1816785", "level3", "desktop_organization", "chenyu"] });
    ...
    afterEach(async ({ device, agent, env, system}) => {
      ...
      await system.exec('dde-dconfig --set -a org.deepin.dde.file-manager -r org.deepin.dde.file-manager.desktop.organizer -k enableOrganizer -v 1', 500);
        ...
        // 清理测试文件
        await system.exec('rm -f ~/Desktop/1816785.txt', 500);
        await system.exec('rm -f ~/Desktop/1816785.xlsx', 500);
        await system.exec('rm -f ~/Desktop/1816785.doc', 500);
    });
