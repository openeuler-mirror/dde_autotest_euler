...
describe('1850173-独立口令和密钥保护的文件保险箱', () => {
  ...
  test('1850173-独立口令和密钥保护的文件保险箱', async ({ device, agent, uos, env, system }) => {    
    ...
    // 预期 3: 右键菜单中有立即上锁选项
    console.log('预期 3: 右键菜单中有立即上锁选项');
    await agent.aiAssert('右键菜单中有立即上锁选项');
    await device.pressKey('Esc');

  }, { timeout: 1200000, tags: ['1850173','level2', 'group standard', 'DITT', 'youwei', 'file-manager', 'vault'] });
});