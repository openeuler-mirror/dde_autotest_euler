/**
 * 用例 PMSID: 1808667
 * 用例标题:【复制远程路径】地址栏上鼠标右键复制ftp路径
 * 生成时间: 2026-03-02 09:50:26
 * 用例编写人：UT000244(李庆玲)
 */

describe('1808667-【复制远程路径】地址栏上鼠标右键复制ftp路径', () => {
    ...
    test('1808667-【复制远程路径】地址栏上鼠标右键复制ftp路径', async ({ device, agent, uos, system, env, clipboard }) => {
    // 挂载FTP并访问FTP
    ...
    await device.typeText('1808667', 500);
    await agent.aiTap('空白区域');
    await agent.aiDoubleClick('1808667文件夹');
    await agent.aiWaitFor('打开1808667文件夹');

    // 步骤1：在文管地址栏选中FTP路径，鼠标右键复制地址
    await agent.aiTap('顶部标签下的地址栏上的1808667文件夹');
    await agent.aiRightClick('顶部标签下的地址栏上的1808667文件夹');
    ...
    await agent.aiAssert(`桌面右侧剪切板中第一行显示ftp://${process.env.FTP_IP}/1808667`);
      
    // 打开文本编辑器，粘贴FTP路径
    await agent.aiTap('任务栏上文本编辑器');
    await agent.aiTap('文本输入框第一行');
    await device.pressKey('Ctrl+V');
    await agent.aiAssert(`显示ftp://${process.env.FTP_IP}/1808667`);

    // 关闭文本编辑器
    await agent.aiTap('文本编辑器标签页的关闭按钮');
    await agent.aiTap('不保存');

    // 删除在ftp上创建的1808667文件夹
    await agent.aiTap('顶部标签下的地址栏上的后退按钮');
    await agent.aiTap('ftp目录的中间空白区域');
    await agent.aiTap('1808667文件夹');
    await agent.aiRightClick('1808667文件夹');
    await agent.aiTap('删除', 500);
    await agent.aiTap('删除');
    await agent.aiAssert('ftp目录下不存在1808667文件夹');
    }, { timeout: 1800000, tags: ["1808667", "level2", "history_navigation", "liqingling"] });
