/**
 * 用例 PMSID: 1808665
 * 用例标题:【复制远程路径】地址栏上鼠标右键编辑sftp路径
 * 生成时间: 2026-03-02 09:50:26
 * 用例编写人：UT000244(李庆玲)
 */

describe('1808665-【复制远程路径】地址栏上鼠标右键编辑sftp路径', () => {
    ...
    test('1808665-【复制远程路径】地址栏上鼠标右键编辑sftp路径', async ({ device, agent, uos, system, env, clipboard }) => {
    // 挂载SFTP并访问SFTP
    ...
    // 在sftp中新建1808665文件夹并双击打开
    await agent.aiTap('sftp目录右侧空白区域');
    await agent.aiRightClick('空白区域', 500);
    await agent.aiTap('新建文件夹');
    await device.typeText('1808665', 500);
    await agent.aiTap('空白区域');
    await agent.aiDoubleClick('1808665文件夹');
    await agent.aiWaitFor('打开1808665文件夹');

    // 步骤1：在文管地址栏选中SFTP路径，鼠标右键复制地址
    await agent.aiTap('地址栏上的1808665文件夹');
    await agent.aiRightClick('地址栏上的1808665文件夹');
    ...
    await agent.aiAssert(`桌面右侧剪切板中第一行显示sftp://${process.env.SFTP_IP}/sftp/upload/1808665`);
      
    // 打开文本编辑器，粘贴SFTP路径
    await agent.aiTap('任务栏上文本编辑器');
    await agent.aiTap('文本输入框第一行');
    await device.pressKey('Ctrl+V');
    await agent.aiAssert(`显示sftp://${process.env.SFTP_IP}/sftp/upload/1808665`);

    // 关闭文本编辑器
    await agent.aiTap('文本编辑器标签页的关闭按钮');
    await agent.aiTap('不保存');

    // 删除sftp上创建的1808665文件夹
    await agent.aiTap('顶部标签下的地址栏上的后退按钮');
    await agent.aiTap('sftp目录的中间空白区域');
    await agent.aiTap('1808665文件夹');
    await agent.aiRightClick('1808665文件夹');
    await agent.aiTap('删除', 500);
    await agent.aiTap('删除');
    await agent.aiAssert('upload目录下不存在1808665文件夹');

    }, { timeout: 1800000, tags: ["1808665", "level2", "history_navigation", "liqingling"] });
