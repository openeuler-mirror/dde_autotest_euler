/**
 * 用例 PMSID: 2002751
 * 用例标题: 扩展名隐藏-取消勾选"显示文件扩展名"后，检查SFTP目录文件名是否隐藏
 * 生成时间: 2026-05-19 12:00:00
 * 用例编写人: UT000649（黄甜）
 */

describe('2002751-扩展名隐藏-SFTP目录', () => {
  ...
  beforeEach(async ({ device, agent, system, uos }) => {
    ...
    await SftpMount(agent, system, device, 1);
    await system.exec(`echo 1 > /run/user/1000/gvfs/sftp:host=${ip}/sftp/upload/2002751.docx`, 500);
    await agent.aiRightClick("文件管理器窗口空白处");
    await agent.aiTap("刷新");
  });

  test('2002751-扩展名隐藏-SFTP目录', async ({ device, agent, uos }) => {
    ...
    await agent.aiAssert("目录下存在2002751文件");

  }, { timeout: 600000, tags: ['2002751', 'level2', 'smoke', 'DITT', 'huangtian'] });
  ...
  afterAll(async ({ uos, agent, device, system }) => {
    ...
    //清除文件
    await system.exec(`rm -rf /run/user/1000/gvfs/sftp:host=${ip}/sftp/upload/2002751*`, 500);
    // 卸载sftp
    const { cleanSftpMounts } = await import(`${caseDir}midscene_dde_file_manager/common/common.ts`);
    await cleanSftpMounts(agent, system, 1);
