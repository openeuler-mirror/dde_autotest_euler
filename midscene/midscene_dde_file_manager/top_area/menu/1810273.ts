/**
 * 用例 PMSID: 1810273
 * 用例标题: 勾选【显示隐藏文件】-在ftp共享目录内“.”开头新建/重命名文件-不弹窗提示
 * 生成时间: 2026-05-26
 * 用例编写人: UT000649（黄甜）
 */
...
describe('1810273-勾选【显示隐藏文件】-在ftp共享目录内“.”开头新建/重命名文件-不弹窗提示', () => {
  ...
  beforeEach(async ({ device, agent, system, uos }) => {
    ...
    await system.exec(`rm -rf /run/user/1000/gvfs/ftp\:host\=${ip}/1810273_test`);
  ...
  test('1810273-勾选【显示隐藏文件】-在ftp共享目录内“.”开头新建/重命名文件-不弹窗提示', async ({ device, agent, uos, system }) => {
    console.log('=== 开始测试：1810273 ===');
    
    const ip = process.env.FTP_IP;
    const ftpPath = `/run/user/1000/gvfs/ftp:host=${ip}`;
    
    console.log('步骤1: 在ftp共享目录内以“.”开头新建文件');
    await system.exec(`mkdir -p /run/user/1000/gvfs/ftp\:host\=${ip}/1810273_test`);
    await agent.aiRightClick('文件管理器窗口空白处');
    await agent.aiTap('刷新');
    ...
    await agent.aiDoubleClick('1810273_test目录');    
    await agent.aiRightClick('文件管理器窗口空白处');
