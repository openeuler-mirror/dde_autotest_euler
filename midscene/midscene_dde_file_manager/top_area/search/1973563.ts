/**
 * 用例 PMSID: 1973563
 * 用例标题: 【搜索】【黑名单配置】在黑名单路径下修改文件内容再搜索
 * 生成时间: 2026-05-13 14:55:00
 * 用例编写人: UT002411（胡戬）
 */
...
describe('1973563-【搜索】【黑名单配置】在黑名单路径下修改文件内容再搜索', () => {
  beforeAll(async ({ device, uos, agent, system }) => {
    ...
    await system.exec(`echo "测试搜索黑名单路径\nblack-path" > ${desktopBlackPath}/black-path.txt`);
  });

  test('1973563-【搜索】【黑名单配置】在黑名单路径下修改文件内容再搜索', async ({ device, agent, uos, system }) => {
    ...
    // 步骤3：将桌面上black-path目录下black-path.txt文件内容中的black-path删除
    console.log('步骤3：修改black-path.txt文件内容，删除black-path关键词');
    ...
    console.log('断言2通过：成功验证黑名单路径下的文件内容修改后无法被搜索');

  }, { timeout: 600000, tags: ['1973563', 'level3', 'search', 'hujian'] });
