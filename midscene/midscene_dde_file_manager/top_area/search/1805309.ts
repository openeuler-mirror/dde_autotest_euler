...
/**
 * 用例 PMSID: 1805309
 * 用例标题: 【搜索】搜索地址前缀补全-访问共享地址后检查多窗口最近访问标签显示 
 * 生成时间: 2026-03-05 20:06:21
 * 用例编写人: UT000193（郑豪）
 */


describe('1805309-【搜索】搜索地址前缀补全-访问共享地址后检查多窗口最近访问标签显示 ', () => {
  ...
  test('1805309-【搜索】搜索地址前缀补全-访问共享地址后检查多窗口最近访问标签显示 ', async ({ device, agent, uos, system }) => {
    await new Promise(resolve => setTimeout(resolve, 5000));
    await agent.aiTap("文件管理器右上角'X'关闭按钮");
    await new Promise(resolve => setTimeout(resolve, 5000));
    // 步骤1：挂载一个共享地址，打开多窗口，输入ip检查下拉框最近标签显示
    await device.pressKey('Super+E');
    await new Promise(resolve => setTimeout(resolve, 1000));
    await device.pressKey('Super+Up');
    await new Promise(resolve => setTimeout(resolve, 1000));
    ...
    // 断言1：多窗口下拉框共享地址显示最近访问标签
    await agent.aiAssert(`下拉框其中一条显示：圆形图标+smb：//+${ip}`);
  }, { timeout: 600000, tags: ['1805309', 'level3', 'search', 'zhenghao'] });
