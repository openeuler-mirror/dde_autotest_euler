...
/**
 * 用例 PMSID: 1810589
 * 用例标题: 【搜索】长文件名功能 - 搜索结果，文件右键打开文件所在位置
 * 生成时间: 2026-03-17 13:26:54
 * 用例编写人: UT000193（郑豪）
 */

describe('1810589-【搜索】长文件名功能 - 搜索结果，文件右键打开文件所在位置', () => {
  // 测试资源文件变量
  const longFileNamePath = `/home/${process.env.TEST_USERNAME}/Desktop`;
  const longFileName = '这是一个非常长的文件名用于测试长文件名功能这是一个非常长的文件名用于测试长文件名功能这是一个非常长的文件名用于测试长文件名功能这是一个非常长的文件名用于测试长文件名功能这是一个非常长的文件名用于测试长文件名功能_1810589.txt';
  const longFolderName = '这是一个非常长的文件夹名用于测试长文件名功能这是一个非常长的文件夹名用于测试长文件名功能这是一个非常长的文件夹名用于测试长文件名功能这是一个非常长的文件夹名用于测试长文件名功能这是一个非常长的文件夹名用于测试长文件名功能_1810589';
  ...
  test('1810589-【搜索】长文件名功能 - 搜索结果，文件右键打开文件所在位置', async ({ device, agent, uos, system }) => {
    // 步骤1：打开文件管理器并搜索长文件名
    ...
    await agent.aiWaitFor('新窗口打开文件夹所在目录');
    
    // 断言1：新窗口打开文件夹所在目录
    await agent.aiAssert(`新窗口显示${longFolderName}文件夹所在目录`);
    
    // 步骤2：选择长文件夹名，右键"打开文件所在位置"
    await device.pressKey('Alt+F4');
    await new Promise(resolve => setTimeout(resolve, 1000));
    await agent.aiRightClick(`搜索结果中的txt文件: ${longFileName}`);
    await agent.aiTap('右键菜单中的"打开文件所在位置"选项');
    await agent.aiWaitFor('新窗口打开文件所在目录');

    // 预期2：新窗口打开文件所在目录
    await agent.aiAssert(`新窗口显示${longFileName}文件所在目录`);
  }, { timeout: 600000, tags: ['1810589', 'level3', 'remote', 'search', 'zhenghao'] });
